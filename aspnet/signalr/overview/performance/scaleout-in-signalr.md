...
- [서버 브로드캐스트](../getting-started/tutorial-server-broadcast-with-signalr.md) (예: 주식 시세 표시기): 서버 메시지 전송 되는 속도 제어 하므로이 시나리오에 적합 백플레인 작동 합니다.
- [클라이언트-](../getting-started/tutorial-getting-started-with-signalr.md) (채팅 예):이 시나리오에서는 클라이언트의 수를 사용 하 여 메시지 수가 조정 하는 경우 백플레인에서 병목 지점이 될 수 있습니다; 그리고 즉, 메시지의 속도 증가 하면 비례적으로 더 많은 클라이언트 연결 합니다.
