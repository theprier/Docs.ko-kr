---
uid: signalr/overview/performance/index
title: SignalR 성능 | Microsoft Docs
author: rick-anderson
description: SignalR 응용 프로그램의 성능을 최대화 하는 방법.
ms.author: aspnetcontent
manager: wpickett
ms.date: 09/19/2014
ms.topic: article
ms.assetid: fda81611-b67b-4c62-915e-8adc1924a401
ms.technology: dotnet-signalr
ms.prod: .net-framework
msc.legacyurl: /signalr/overview/performance
msc.type: chapter
ms.openlocfilehash: efbdb0a329972a56d8518c35dc1ead1bcb8305e6
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/10/2017
ms.locfileid: "26505602"
---
<a name="signalr-performance"></a>SignalR 성능
====================
> SignalR 응용 프로그램의 성능을 최대화 하는 방법.


- [SignalR 성능](signalr-performance.md)
- [SignalR에서 확장 소개](scaleout-in-signalr.md)
- [Azure 서비스 버스에 SignalR 확장](scaleout-with-windows-azure-service-bus.md)
- [Redis와 함께 SignalR 확장](scaleout-with-redis.md)
- [SQL Server와 함께 SignalR 확장](scaleout-with-sql-server.md)
- [SignalR 연결 밀도 크랭크를 사용 하 여 테스트](signalr-connection-density-testing-with-crank.md)
- [SignalR 성능 카운터를 사용 하 여 Azure 웹 역할에서](using-signalr-performance-counters-in-an-azure-web-role.md)
