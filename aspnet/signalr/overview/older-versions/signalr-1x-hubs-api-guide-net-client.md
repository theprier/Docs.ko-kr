...
프록시 개체는 스레드로부터 안전 합니다. 호출 하는 경우 실제로 `HubConnection.CreateHubProxy` 으로 여러 번 동일한 `hubName`, 캐시 같은 `IHubProxy` 개체입니다.
...
자세한 내용은 참조 [이해 하 고 SignalR에서 연결 수명 이벤트를 처리](../guide-to-the-api/handling-connection-lifetime-events.md)합니다.
