...
Visual Studio 2017에 ASP.NET 웹 앱을 개발 하려면 다음을 설치 합니다 **ASP.NET 및 웹 개발** 워크 로드.
...
3. 선택 된 `GetHtml` 메서드 목록에서 한 다음 Enter 키를 누릅니다. IntelliSense 메서드를 자동으로 채웁니다. (C#의 모든 메서드를 추가 해야 하는 대로 `()` 메서드 이후에 문자입니다.) 에 대 한 완성 된 코드는 `GetHtml` 메서드는 다음 예제와 같습니다.
