...
Razor 구문은 웹 페이지에서 서버 기반 코드를 포함 하기 위한 단순 프로그래밍 구문입니다. Razor 구문을 사용 하 여 웹 페이지에는 두 종류의 콘텐츠: 클라이언트가 콘텐츠 및 서버 코드입니다. 클라이언트 콘텐츠는 웹 페이지에는 항목: HTML 태그 (요소), CSS 등의 정보 아마도 JavaScript 및 일반 텍스트와 같은 일부 클라이언트 스크립트를 스타일입니다.
