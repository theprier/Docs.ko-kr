...
12. 에 대 한 **데이터 형식**, 선택 &quot;nvarchar&quot; 길이 50으로 설정 합니다. *var* 의 일부가 `nvarchar` 이 열에 대 한 데이터가 필요에 따라 크기가 레코드 간에 다를 수 있습니다 하는 문자열을 될 것 이라는 데이터베이스에 알려 줍니다. (의  *n*  나타내는 접두사 *national*을 나타내는 필드는 알파벳 이나 시스템 &#8212;쓰기를 나타내는입니다; 즉 문자 데이터를 보유할 수, 필드에 유니코드 데이터)
...
3. 기존 태그를 다음 코드로 바꿉니다.
    ...
    열은 첫 번째 코드 블록에는 *SmallBakery.sdf* 앞에서 만든 파일 (데이터베이스). `Database.Open` 메서드 가정 하는 *.sdf* 파일은 웹 사이트의 *앱\_데이터* 폴더입니다. (사라졌는지 지정할 필요가 없습니다는 *.sdf* 확장 &#8212; 이렇게 하면 실제로 `Open` 메서드가 작동 하지 않습니다.)
    ...
    문에서 `Product` 쿼리에 테이블을 식별 합니다. `*` 문자 지정 하면 쿼리 테이블에서 모든 열을 반환 해야 합니다. (또한를 나열할 수 열 개별적으로 열 중 일부만 표시 하려는 경우 쉼표로 구분 합니다.) `Order By` 절 나타냅니다는 데이터를 정렬 해야 하는 방법 &#8212; 하 여이 경우는 *이름* 열입니다. 즉, 데이터의 값에 따라 사전순으로 정렬 되는 *이름* 각 행에 대 한 열입니다.
...
> 
> 에서 설명한 것 처럼는 `Database.Open` 메서드를 사용 하면 데이터베이스 이름 또는 연결 문자열을 전달 하 고 사용 하는 그림 합니다. 배포할 때이 매우 유용할 (게시) 웹 사이트입니다. 사용할 수는 *.sdf* 파일에 *앱\_데이터* 개발 하 고 사이트를 테스트 하는 경우 폴더입니다. 프로덕션 서버에 사이트를 이동 하면에서 연결 문자열을 사용할 수 있습니다는 *Web.config* 와 같은 이름을 가진 파일을 여 *.sdf* 파일이 있지만 호스팅 공급자의 데이터베이스 & #를 가리키는 8212; 없이 코드를 변경 합니다.
> 
