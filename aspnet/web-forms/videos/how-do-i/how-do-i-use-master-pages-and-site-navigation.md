---
uid: web-forms/videos/how-do-i/how-do-i-use-master-pages-and-site-navigation
title: "[어떻게 할까요?] 사용 하 여 마스터 페이지 및 사이트 탐색 | Microsoft 문서"
author: microsoft
description: "ASP.NET 2.0 마스터 페이지를 사용 하 여 모든 해당 페이지에 걸쳐 일관 된 레이아웃 및 디자인 된 웹 사이트를 만듭니다. 탐색 웹 사이트에 추가 하는 것이 얼마나 쉬운지 보기..."
ms.author: aspnetcontent
manager: wpickett
ms.date: 01/11/2006
ms.topic: article
ms.assetid: e598bb3b-4e2e-4a71-a255-3ee89061ce7a
ms.technology: dotnet-webforms
ms.prod: .net-framework
msc.legacyurl: /web-forms/videos/how-do-i/how-do-i-use-master-pages-and-site-navigation
msc.type: video
ms.openlocfilehash: b9532de57a06fa9ef5a0f1cb5d7f66a75307fe99
ms.sourcegitcommit: 9a9483aceb34591c97451997036a9120c3fe2baf
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/10/2017
---
<a name="how-do-i-use-master-pages-and-site-navigation"></a>[어떻게 할까요?] 사용 하 여 마스터 페이지 및 사이트 탐색
====================
여 [Microsoft](https://github.com/microsoft)

ASP.NET 2.0 마스터 페이지를 사용 하 여 모든 해당 페이지에 걸쳐 일관 된 레이아웃 및 디자인 된 웹 사이트를 만듭니다. SiteMapPath 제어 및 탐색을 새 TreeView를 사용 하 여 웹 사이트에 추가 하려면 얼마나 쉬운지를 참조 하십시오.

[&#9654; (13 분) 비디오를 시청 하세요](https://channel9.msdn.com/Blogs/ASP-NET-Site-Videos/how-do-i-use-master-pages-and-site-navigation)
