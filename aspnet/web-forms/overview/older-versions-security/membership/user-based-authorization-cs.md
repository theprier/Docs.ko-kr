...
> [!NOTE]
> `DirectoryInfo` 및 `FileInfo` 에서 찾을 수 없는 클래스는 [ `System.IO` 네임 스페이스](https://msdn.microsoft.com/en-us/library/system.io.aspx)합니다. 따라서가를 앞에 이러한 클래스 이름은 네임 스페이스 이름으로, 아니면 클래스 파일에 가져온 네임 스페이스 (통해 `using System.IO`).
