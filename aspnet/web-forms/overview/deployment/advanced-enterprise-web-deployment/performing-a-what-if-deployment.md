...
에 설명 된 대로 [웹 패키지 배포](../web-deployment-in-the-enterprise/deploying-web-packages.md), 웹 배포를 사용 하 여 두 가지 방법으로 웹 패키지를 배포 하려면&#x2014;직접 또는 실행 하 여 MSDeploy.exe 명령줄 유틸리티를 사용 하 여는 *. deploy.cmd* 파일 빌드 프로세스를 생성 합니다.
...
마찬가지로, 다음 대상 VSDBCMD 유틸리티를 사용 하 여 데이터베이스를 배포 합니다. 기본적으로는 **d d** 스위치는 포함 되지 않습니다. 즉, VSDBCMD 배포 스크립트를 생성 하는 있지만 데이터베이스를 배포 하지 것입니다&#x2014;즉, "what-if" 시나리오입니다. 경우는 **WhatIf** 속성으로 설정 되지 않은 **true**, **d d** 스위치 추가 되 고 VSDBCMD 데이터베이스를 배포 합니다.
