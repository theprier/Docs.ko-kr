...
## <a name="adding-order-tracking"></a><span data-ttu-id="9a9d3-127">주문 추적 추가</span><span class="sxs-lookup"><span data-stu-id="9a9d3-127">Adding Order Tracking</span></span>
...
1. <span data-ttu-id="9a9d3-141">**솔루션 탐색기**찾기 및 열기는 *ProductContext.cs* 파일입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-141">In **Solution Explorer**, find and open the *ProductContext.cs* file.</span></span>
...
8. <span data-ttu-id="9a9d3-196">**Ctrl+F5**를 눌러 응용 프로그램을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-196">Press **Ctrl+F5** to run the application.</span></span> <span data-ttu-id="9a9d3-197">Visual Studio에는 SSL 경고를 방지할 수 있도록 하는 옵션이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-197">Visual Studio will display an option to allow you to avoid SSL warnings.</span></span>
...
8. <span data-ttu-id="9a9d3-232">설정의 **자바 스크립트 원본** 이 자습서의 앞부분에서 사용한 SSL URL에 (`https://localhost:44300/` 다른 SSL 프로젝트 생성 하지 않는 한).</span><span class="sxs-lookup"><span data-stu-id="9a9d3-232">Set the **Authorized JavaScript Origins** to the SSL URL you used earlier in this tutorial (`https://localhost:44300/` unless you've created other SSL projects).</span></span>   
   <span data-ttu-id="9a9d3-233">이 URL에는 응용 프로그램에 대 한 원본입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-233">This URL is the origin for your application.</span></span> <span data-ttu-id="9a9d3-234">이 샘플에 대 한만 localhost 테스트 URL을 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-234">For this sample, you will only enter the localhost test URL.</span></span> <span data-ttu-id="9a9d3-235">그러나 여러 개의 Url이 localhost와 프로덕션에 대 한 설명 하기 위해 입력할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-235">However, you can enter multiple URLs to account for localhost and production.</span></span>
...
18. <span data-ttu-id="9a9d3-255">자격 증명을 입력 한 후 방금 만든 웹 응용 프로그램에 권한을 부여 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-255">After you enter your credentials, you will be prompted to give permissions to the web application you just created.</span></span>  
...
1. <span data-ttu-id="9a9d3-287">**솔루션 탐색기**, 찾을 *논리* 폴더를 열고 다음의 *ShoppingCartActions.cs* 클래스 파일입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-287">In **Solution Explorer**, find the *Logic* folder and open the *ShoppingCartActions.cs* class file.</span></span>
...
### <a name="create-paylpal-test-accounts"></a><span data-ttu-id="9a9d3-300">PaylPal 테스트 계정 만들기</span><span class="sxs-lookup"><span data-stu-id="9a9d3-300">Create PaylPal Test Accounts</span></span>
...
1. <span data-ttu-id="9a9d3-333">Visual Studio 내에서 Wingtip Toys 샘플 응용 프로그램을 마우스 오른쪽 단추로 클릭는 **논리** 폴더 및 다음 선택 **추가**  - &gt; **새 항목**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-333">In the Wingtip Toys sample application within Visual Studio, right-click the **Logic** folder and then select **Add** -&gt; **New Item**.</span></span>   
...
5. <span data-ttu-id="9a9d3-378">또한는 *ShoppingCart.aspx.cs* 파일에 대 한 참조를 추가 합니다는 `CheckoutBtn`새 이미지 단추를 다음과 같이 참조 되도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-378">Also in the *ShoppingCart.aspx.cs* file, add a reference to the `CheckoutBtn`, so that the new image button is referenced as follows:</span></span>  

    [!code-csharp[Main](checkout-and-payment-with-paypal/samples/sample18.cs?highlight=18)]
6. <span data-ttu-id="9a9d3-379">둘 다에 변경 내용을 저장는 *ShoppingCart.aspx* 파일 및 *ShoppingCart.aspx.cs* 파일입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-379">Save your changes to both the *ShoppingCart.aspx* file and the *ShoppingCart.aspx.cs* file.</span></span>
7. <span data-ttu-id="9a9d3-380">메뉴에서 선택 **디버그**-&gt;**빌드 WingtipToys**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-380">From the menu, select **Debug**-&gt;**Build WingtipToys**.</span></span>  
   <span data-ttu-id="9a9d3-381">프로젝트를 다시 작성 됩니다 새로 추가 된 **ImageButton** 제어 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-381">The project will be rebuilt with the newly added **ImageButton** control.</span></span>

### <a name="send-purchase-details-to-paypal"></a><span data-ttu-id="9a9d3-382">PayPal 구매 정보 보내기</span><span class="sxs-lookup"><span data-stu-id="9a9d3-382">Send Purchase Details to PayPal</span></span>

<span data-ttu-id="9a9d3-383">사용자가 클릭할 때는 **체크 아웃** 쇼핑 카트 페이지에서 단추 (*ShoppingCart.aspx*), 구매 프로세스를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-383">When the user clicks the **Checkout** button on the shopping cart page (*ShoppingCart.aspx*), they'll begin the purchase process.</span></span> <span data-ttu-id="9a9d3-384">다음 코드는 제품을 구매 하는 데 필요한 첫 번째 PayPal 함수를 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-384">The following code calls the first PayPal function needed to purchase products.</span></span>

1. <span data-ttu-id="9a9d3-385">*체크 아웃* 이라는 폴더를 열고 코드 숨김 파일 *CheckoutStart.aspx.cs*합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-385">From the *Checkout* folder, open the code-behind file named *CheckoutStart.aspx.cs*.</span></span>   
   <span data-ttu-id="9a9d3-386">코드 숨김 파일을 열 수 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-386">Be sure to open the code-behind file.</span></span>
2. <span data-ttu-id="9a9d3-387">기존 코드를 다음으로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-387">Replace the existing code with the following:</span></span>   

    [!code-csharp[Main](checkout-and-payment-with-paypal/samples/sample19.cs)]

<span data-ttu-id="9a9d3-388">응용 프로그램의 사용자가 클릭할 때는 **체크 아웃** 쇼핑 카트 페이지를 브라우저에서 단추도 이동 되는 *CheckoutStart.aspx* 페이지.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-388">When the user of the application clicks the **Checkout** button on the shopping cart page, the browser will navigate to the *CheckoutStart.aspx* page.</span></span> <span data-ttu-id="9a9d3-389">경우는 *CheckoutStart.aspx* 페이지 로드 되는 `ShortcutExpressCheckout` 메서드를 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-389">When the *CheckoutStart.aspx* page loads, the `ShortcutExpressCheckout` method is called.</span></span> <span data-ttu-id="9a9d3-390">이 시점에서 사용자는 PayPal 테스트 웹 사이트에 전송 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-390">At this point, the user is transferred to the PayPal testing web site.</span></span> <span data-ttu-id="9a9d3-391">PayPal 사이트에서 사용자 입력 PayPal 자격 증명, 구매 정보를 검토, PayPal 계약에 동의 하 고 Wingtip Toys 샘플 응용 프로그램에 반환 위치는 `ShortcutExpressCheckout` 메서드가 완료 되 면입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-391">On the PayPal site, the user enters their PayPal credentials, reviews the purchase details, accepts the PayPal agreement and returns to the Wingtip Toys sample application where the `ShortcutExpressCheckout` method completes.</span></span> <span data-ttu-id="9a9d3-392">경우는 `ShortcutExpressCheckout` 메서드가 완료 되 면 사용자를 리디렉션할 수는 *CheckoutReview.aspx* 에 지정 된 페이지는 `ShortcutExpressCheckout` 메서드.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-392">When the `ShortcutExpressCheckout` method is complete, it will redirect the user to the *CheckoutReview.aspx* page specified in the `ShortcutExpressCheckout` method.</span></span> <span data-ttu-id="9a9d3-393">이 사용자를 Wingtip Toys 샘플 응용 프로그램 내에서 주문 정보를 검토할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-393">This allows the user to review the order details from within the Wingtip Toys sample application.</span></span>

### <a name="review-order-details"></a><span data-ttu-id="9a9d3-394">주문 상세 정보 검토</span><span class="sxs-lookup"><span data-stu-id="9a9d3-394">Review Order Details</span></span>

<span data-ttu-id="9a9d3-395">PayPal에서 반환 된 후의 *CheckoutReview.aspx* Wingtip Toys 샘플 응용 프로그램의 페이지에는 주문 세부 정보가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-395">After returning from PayPal, the *CheckoutReview.aspx* page of the Wingtip Toys sample application displays the order details.</span></span> <span data-ttu-id="9a9d3-396">이 페이지는 제품을 구매 하기 전에 주문 세부 정보를 검토할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-396">This page allows the user to review the order details before purchasing the products.</span></span> <span data-ttu-id="9a9d3-397">*CheckoutReview.aspx* 페이지를 다음과 같이 만들 수 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-397">The *CheckoutReview.aspx* page must be created as follows:</span></span>

1. <span data-ttu-id="9a9d3-398">에 *체크 아웃* 이라는 폴더를 열고 페이지 *CheckoutReview.aspx*합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-398">In the *Checkout* folder, open the page named *CheckoutReview.aspx*.</span></span>
2. <span data-ttu-id="9a9d3-399">기존 태그를 다음 코드로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-399">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample20.aspx)]
3. <span data-ttu-id="9a9d3-400">명명 된 코드 숨김 페이지를 열고 *CheckoutReview.aspx.cs* 기존 코드는 다음과 같이 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-400">Open the code-behind page named *CheckoutReview.aspx.cs* and replace the existing code with the following:</span></span>   

    [!code-csharp[Main](checkout-and-payment-with-paypal/samples/sample21.cs)]

<span data-ttu-id="9a9d3-401">**DetailsView** 컨트롤은 PayPal에서 반환 된 주문 정보를 표시 하는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-401">The **DetailsView** control is used to display the order details that have been returned from PayPal.</span></span> <span data-ttu-id="9a9d3-402">위의 코드도 적용할 것 이므로 데이터베이스에 주문 세부 정보를 저장 하는 또한는 `OrderDetail` 개체입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-402">Also, the above code saves the order details to the Wingtip Toys database as an `OrderDetail` object.</span></span> <span data-ttu-id="9a9d3-403">사용자가 클릭할 때는 **전체 Order** 리디렉션됩니다 단추는 *CheckoutComplete.aspx* 페이지.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-403">When the user clicks on the **Complete Order** button, they are redirected to the *CheckoutComplete.aspx* page.</span></span>

> [!NOTE] 
> 
> <span data-ttu-id="9a9d3-404">**팁**</span><span class="sxs-lookup"><span data-stu-id="9a9d3-404">**Tip**</span></span>
> 
> <span data-ttu-id="9a9d3-405">태그에는 *CheckoutReview.aspx* 페이지에서 다음에 유의 `<ItemStyle>` 태그 내에 있는 항목의 스타일을 변경 하는를 사용 하는 **DetailsView** 페이지의 아래쪽에 있는 컨트롤입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-405">In the markup of the *CheckoutReview.aspx* page, notice that the `<ItemStyle>` tag is used to change the style of the items within the **DetailsView** control near the bottom of the page.</span></span> <span data-ttu-id="9a9d3-406">페이지를 확인 하 여 **디자인 보기** (선택 하 여 **디자인** Visual Studio의 왼쪽된 아래 모서리에), 다음을 선택 하는 **DetailsView** 컨트롤을 선택 **스마트 태그** (위쪽 화살표 아이콘 컨트롤의 오른쪽)를 볼 수는 **DetailsView 작업**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-406">By viewing the page in **Design View** (by selecting **Design** at the lower left corner of Visual Studio), then selecting the **DetailsView** control, and selecting the **Smart Tag** (the arrow icon at the top right of the control), you will be able to see the **DetailsView Tasks**.</span></span>
> 
> ![체크 아웃 및 PayPal-지불 필드 편집](checkout-and-payment-with-paypal/_static/image18.png)
> 
> <span data-ttu-id="9a9d3-408">선택 하 여 **필드 편집**, **필드** 대화 상자가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-408">By selecting **Edit Fields**, the **Fields** dialog box will appear.</span></span> <span data-ttu-id="9a9d3-409">이 대화 상자에서 쉽게 제어할 수 있습니다 시각적 속성을 같은 **ItemStyle**의 **DetailsView** 제어 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-409">In this dialog box you can easily control the visual properties, such as **ItemStyle**, of the **DetailsView** control.</span></span>
> 
> ![체크 아웃 및 PayPal-필드 대화 상자를 사용 하 여 상환액](checkout-and-payment-with-paypal/_static/image19.png)


### <a name="complete-purchase"></a><span data-ttu-id="9a9d3-411">구매를 완료 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-411">Complete Purchase</span></span>

<span data-ttu-id="9a9d3-412">*CheckoutComplete.aspx* 페이지 PayPal을 통한 구매를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-412">*CheckoutComplete.aspx* page makes the purchase from PayPal.</span></span> <span data-ttu-id="9a9d3-413">사용자를 클릭 해야 위에서 설명 했 듯이 **전체 Order** 응용 프로그램으로 이동 되 전에 단추는 *CheckoutComplete.aspx* 페이지.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-413">As mentioned above, the user must click on the **Complete Order** button before the application will navigate to the *CheckoutComplete.aspx* page.</span></span>

1. <span data-ttu-id="9a9d3-414">에 *체크 아웃* 이라는 폴더를 열고 페이지 *CheckoutComplete.aspx*합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-414">In the *Checkout* folder, open the page named *CheckoutComplete.aspx*.</span></span>
2. <span data-ttu-id="9a9d3-415">기존 태그를 다음 코드로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-415">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample22.aspx)]
3. <span data-ttu-id="9a9d3-416">명명 된 코드 숨김 페이지를 열고 *CheckoutComplete.aspx.cs* 기존 코드는 다음과 같이 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-416">Open the code-behind page named *CheckoutComplete.aspx.cs* and replace the existing code with the following:</span></span>   

    [!code-csharp[Main](checkout-and-payment-with-paypal/samples/sample23.cs)]

<span data-ttu-id="9a9d3-417">경우는 *CheckoutComplete.aspx* 페이지가 로드 되는 `DoCheckoutPayment` 메서드를 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-417">When the *CheckoutComplete.aspx* page is loaded, the `DoCheckoutPayment` method is called.</span></span> <span data-ttu-id="9a9d3-418">앞에서 언급 된 `DoCheckoutPayment` 메서드 PayPal 테스트 환경에서 구매를 완료 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-418">As mentioned earlier, the `DoCheckoutPayment` method completes the purchase from the PayPal testing environment.</span></span> <span data-ttu-id="9a9d3-419">PayPal 구매 주문에 대 완료 되 면는 *CheckoutComplete.aspx* 페이지 표시 지불 트랜잭션 `ID` 구매자 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-419">Once PayPal has completed the purchase of the order, the *CheckoutComplete.aspx* page displays a payment transaction `ID` to the purchaser.</span></span>

### <a name="handle-cancel-purchase"></a><span data-ttu-id="9a9d3-420">구매 취소를 처리 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-420">Handle Cancel Purchase</span></span>

<span data-ttu-id="9a9d3-421">사용자의 구입을 취소 하려는 경우, 이러한 연결 됩니다는 *CheckoutCancel.aspx* 페이지에 표시 되는 경우 해당 순서 취소 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-421">If the user decides to cancel the purchase, they will be directed to the *CheckoutCancel.aspx* page where they will see that their order has been cancelled.</span></span>

1. <span data-ttu-id="9a9d3-422">명명 된 페이지를 열고 *CheckoutCancel.aspx* 에 *체크 아웃* 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-422">Open the page named *CheckoutCancel.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="9a9d3-423">기존 태그를 다음 코드로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-423">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample24.aspx)]

### <a name="handle-purchase-errors"></a><span data-ttu-id="9a9d3-424">구매 오류 처리</span><span class="sxs-lookup"><span data-stu-id="9a9d3-424">Handle Purchase Errors</span></span>

<span data-ttu-id="9a9d3-425">구매 프로세스 동안 발생 한 오류에서 처리 되는 *CheckoutError.aspx* 페이지.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-425">Errors during the purchase process will be handled by the *CheckoutError.aspx* page.</span></span> <span data-ttu-id="9a9d3-426">관련 코드는 *CheckoutStart.aspx* 페이지는 *CheckoutReview.aspx* 페이지 및 *CheckoutComplete.aspx* 페이지 각 리디렉션되는  *CheckoutError.aspx* 페이지에서 오류가 발생 하는 경우.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-426">The code-behind of the *CheckoutStart.aspx* page, the *CheckoutReview.aspx* page, and the *CheckoutComplete.aspx* page will each redirect to the *CheckoutError.aspx* page if an error occurs.</span></span>

1. <span data-ttu-id="9a9d3-427">명명 된 페이지를 열고 *CheckoutError.aspx* 에 *체크 아웃* 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-427">Open the page named *CheckoutError.aspx* in the *Checkout* folder.</span></span>
2. <span data-ttu-id="9a9d3-428">기존 태그를 다음 코드로 바꿉니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-428">Replace the existing markup with the following:</span></span>   

    [!code-aspx[Main](checkout-and-payment-with-paypal/samples/sample25.aspx)]

<span data-ttu-id="9a9d3-429">*CheckoutError.aspx* 결제 과정 중에 오류가 발생 하는 경우 페이지 오류 세부 정보에 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-429">The *CheckoutError.aspx* page is displayed with the error details when an error occurs during the checkout process.</span></span>

## <a name="running-the-application"></a><span data-ttu-id="9a9d3-430">응용 프로그램 실행</span><span class="sxs-lookup"><span data-stu-id="9a9d3-430">Running the Application</span></span>

<span data-ttu-id="9a9d3-431">제품을 구매 하는 방법을 보려면 응용 프로그램을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-431">Run the application to see how to purchase products.</span></span> <span data-ttu-id="9a9d3-432">Note 있습니다 실행 될 것으로 PayPal에서 테스트 환경.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-432">Note that you will be running in the PayPal testing environment.</span></span> <span data-ttu-id="9a9d3-433">실제 미지불 교환 되 고 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-433">No actual money is being exchanged.</span></span>

1. <span data-ttu-id="9a9d3-434">모든 Visual Studio에서 파일을 저장 했는지 확인.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-434">Make sure all your files are saved in Visual Studio.</span></span>
2. <span data-ttu-id="9a9d3-435">웹 브라우저를 열고 이동 [ https://developer.paypal.com ](https://developer.paypal.com/)합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-435">Open a Web browser and navigate to [https://developer.paypal.com](https://developer.paypal.com/).</span></span>
3. <span data-ttu-id="9a9d3-436">이 자습서의 앞부분에서 만든 PayPal 개발자 계정으로 로그인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-436">Login with your PayPal developer account that you created earlier in this tutorial.</span></span>  
   <span data-ttu-id="9a9d3-437">PayPal의 개발자 샌드박스에 대 한 동시에 로그인 해야 [ https://developer.paypal.com ](https://developer.paypal.com/) express 체크 아웃을 테스트 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-437">For PayPal's developer sandbox, you need to be logged in at [https://developer.paypal.com](https://developer.paypal.com/) to test express checkout.</span></span> <span data-ttu-id="9a9d3-438">이 PayPal의 샌드박스 하지 PayPal의 실제 환경에 테스트에 적용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-438">This only applies to PayPal's sandbox testing, not to PayPal's live environment.</span></span>
4. <span data-ttu-id="9a9d3-439">키를 눌러 Visual Studio에서 **F5** Wingtip Toys 샘플 응용 프로그램을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-439">In Visual Studio, press **F5** to run the Wingtip Toys sample application.</span></span>  
   <span data-ttu-id="9a9d3-440">데이터베이스 다시 작성 한 후 브라우저 열리며 표시는 *Default.aspx* 페이지.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-440">After the database rebuilds, the browser will open and show the *Default.aspx* page.</span></span>
5. <span data-ttu-id="9a9d3-441">3 가지 제품을 장바구니에 "Cars"와 같은 제품 범주를 선택 하 고 다음을 클릭 하 여 추가 **카트에 추가** 옆에 있는 각 제품입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-441">Add three different products to the shopping cart by selecting the product category, such as "Cars" and then clicking **Add to Cart** next to each product.</span></span>  
   <span data-ttu-id="9a9d3-442">쇼핑 카트는 선택한 제품을 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-442">The shopping cart will display the product you have selected.</span></span>
6. <span data-ttu-id="9a9d3-443">클릭는 **PayPal** 단추를 체크 아웃 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-443">Click the **PayPal** button to checkout.</span></span> 

    ![체크 아웃 및 PayPal-카트 지불](checkout-and-payment-with-paypal/_static/image20.png)

   <span data-ttu-id="9a9d3-445">체크 아웃 Wingtip Toys 샘플 응용 프로그램에 대 한 사용자 계정이 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-445">Checking out will require that you have a user account for the Wingtip Toys sample application.</span></span>
7. <span data-ttu-id="9a9d3-446">클릭는 **Google** 기존 gmail.com 전자 메일 계정으로 로그인 페이지의 오른쪽에 링크 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-446">Click the **Google** link on the right of the page to log in with an existing gmail.com email account.</span></span>  
   <span data-ttu-id="9a9d3-447">gmail.com 계정이 없는 경우 테스트용으로에서 하나를 만들 수 있습니다 [www.gmail.com](https://www.gmail.com/)합니다. 또한 "등록"을 클릭 하 여 표준 로컬 계정을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-447">If you do not have a gmail.com account, you can create one for testing purposes at [www.gmail.com](https://www.gmail.com/). You can also use a standard local account by clicking "Register".</span></span> 

    ![체크 아웃 및 PayPal-지불 로그인](checkout-and-payment-with-paypal/_static/image21.png)
8. <span data-ttu-id="9a9d3-449">Gmail 계정 및 암호를 사용 하 여 로그인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-449">Sign in with your gmail account and password.</span></span> 

    ![체크 아웃 및 PayPal-Gmail 로그인으로 지불](checkout-and-payment-with-paypal/_static/image22.png)
9. <span data-ttu-id="9a9d3-451">클릭는 **로그인** gmail 계정 Wingtip Toys 샘플 응용 프로그램 사용자 이름으로 등록 하는 단추입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-451">Click the **Log in** button to register your gmail account with your Wingtip Toys sample application user name.</span></span> 

    ![체크 아웃 및 PayPal-레지스터 계정 지불](checkout-and-payment-with-paypal/_static/image23.png)
10. <span data-ttu-id="9a9d3-453">PayPal 테스트 사이트에 추가 프로그램 **구매자** 전자 메일 주소 및이 자습서의 앞부분에서 만든 암호를 한 다음 클릭는 **로그인** 단추입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-453">On the PayPal test site, add your **buyer** email address and password that you created earlier in this tutorial, then click the **Log In** button.</span></span> 

    ![체크 아웃 및 PayPal-PayPal 로그인으로 지불](checkout-and-payment-with-paypal/_static/image24.png)
11. <span data-ttu-id="9a9d3-455">PayPal 정책에 동의 하 고 클릭는 **동 및 계속** 단추입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-455">Agree to the PayPal policy and click the **Agree and Continue** button.</span></span>  
    <span data-ttu-id="9a9d3-456">이 페이지는만 참고가 평가용 계정을 사용 하 여 처음으로 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-456">Note that this page is only displayed the first time you use this PayPal account.</span></span> <span data-ttu-id="9a9d3-457">이것은 테스트 계정, 실제 미지불 교환 note 다시 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-457">Again note that this is a test account, no real money is exchanged.</span></span> 

    ![체크 아웃 및 PayPal-PayPal 정책으로 지불](checkout-and-payment-with-paypal/_static/image25.png)
12. <span data-ttu-id="9a9d3-459">테스트 환경 검토 페이지 및 클릭 PayPal에서 주문 정보를 검토 **계속**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-459">Review the order information on the PayPal testing environment review page and click **Continue**.</span></span> 

    ![체크 아웃 및 지불 PayPal-정보 검토](checkout-and-payment-with-paypal/_static/image26.png)
13. <span data-ttu-id="9a9d3-461">에 *CheckoutReview.aspx* 페이지 주문 금액을 확인 하 고 생성 된 배송 주소를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-461">On the *CheckoutReview.aspx* page, verify the order amount and view the generated shipping address.</span></span> <span data-ttu-id="9a9d3-462">클릭는 **전체 Order** 단추입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-462">Then, click the **Complete Order** button.</span></span> 

    ![체크 아웃 및 지불 PayPal-순서 검토](checkout-and-payment-with-paypal/_static/image27.png)
14. <span data-ttu-id="9a9d3-464">**CheckoutComplete.aspx** 지불 트랜잭션 id 페이지가 표시 됩니다</span><span class="sxs-lookup"><span data-stu-id="9a9d3-464">The **CheckoutComplete.aspx** page is displayed with a payment transaction ID.</span></span> 

    ![체크 아웃 및 체크 아웃 완료-PayPal 지불](checkout-and-payment-with-paypal/_static/image28.png)

<a id="ReviewDBWebForms"></a>
## <a name="reviewing-the-database"></a><span data-ttu-id="9a9d3-466">데이터베이스를 검토합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-466">Reviewing the Database</span></span>

<span data-ttu-id="9a9d3-467">응용 프로그램을 실행 한 후 Wingtip Toys 샘플 응용 프로그램 데이터베이스에 업데이트 된 데이터를 검토 하 여 응용 프로그램에 성공적으로 제품의 구매 기록 볼 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-467">By reviewing the updated data in the Wingtip Toys sample application database after running the application, you can see that the application successfully recorded the purchase of the products.</span></span>

<span data-ttu-id="9a9d3-468">에 포함 된 데이터를 검사할 수는 *Wingtiptoys.mdf* 사용 하 여 데이터베이스 파일의 **데이터베이스 탐색기** 창 (**서버 탐색기** Visual Studio의 창) 수행한 것 처럼 이 자습서 시리즈의 앞부분에 나오는.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-468">You can inspect the data contained in the *Wingtiptoys.mdf* database file by using the **Database Explorer** window (**Server Explorer** window in Visual Studio) as you did earlier in this tutorial series.</span></span>

1. <span data-ttu-id="9a9d3-469">계속 열려 있으면 브라우저 창을 닫습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-469">Close the browser window if it is still open.</span></span>
2. <span data-ttu-id="9a9d3-470">Visual Studio에서 선택 된 **모든 파일 표시** 맨 위에 있는 아이콘 **솔루션 탐색기** 를 확장할 수 있도록는 **앱\_데이터** 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-470">In Visual Studio, select the **Show All Files** icon at the top of **Solution Explorer** to allow you to expand the **App\_Data** folder.</span></span>
3. <span data-ttu-id="9a9d3-471">확장 된 **앱\_데이터** 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-471">Expand the **App\_Data** folder.</span></span>  
 <span data-ttu-id="9a9d3-472">선택 해야는 **모든 파일 표시** 폴더에 대 한 아이콘입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-472">You may need to select the **Show All Files** icon for the folder.</span></span>
4. <span data-ttu-id="9a9d3-473">마우스 오른쪽 단추로 클릭는 *Wingtiptoys.mdf* 데이터베이스 파일 및 선택 **열려**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-473">Right-click the *Wingtiptoys.mdf* database file and select **Open**.</span></span>  
    <span data-ttu-id="9a9d3-474">**서버 탐색기** 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-474">**Server Explorer** is displayed.</span></span>
5. <span data-ttu-id="9a9d3-475">확장 된 **테이블** 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-475">Expand the **Tables** folder.</span></span>
6. <span data-ttu-id="9a9d3-476">마우스 오른쪽 단추로 클릭는 **Orders**테이블을 선택한 **테이블 데이터 표시**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-476">Right-click the **Orders**table and select **Show Table Data**.</span></span>  
 <span data-ttu-id="9a9d3-477">**Orders** 테이블이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-477">The **Orders** table is displayed.</span></span>
7. <span data-ttu-id="9a9d3-478">검토는 **PaymentTransactionID** 성공적인 트랜잭션 확인 하는 열입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-478">Review the **PaymentTransactionID** column to confirm successful transactions.</span></span> 

    ![체크 아웃 및 PayPal-검토 데이터베이스 지불](checkout-and-payment-with-paypal/_static/image29.png)
8. <span data-ttu-id="9a9d3-480">닫기는 **Orders** 테이블 창.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-480">Close the **Orders** table window.</span></span>
9. <span data-ttu-id="9a9d3-481">서버 탐색기에서 마우스 오른쪽 단추로 클릭는 **OrderDetails** 테이블을 선택한 **테이블 데이터 표시**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-481">In the Server Explorer, right-click the **OrderDetails** table and select **Show Table Data**.</span></span>
10. <span data-ttu-id="9a9d3-482">검토는 `OrderId` 및 `Username` 값에 **OrderDetails** 테이블입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-482">Review the `OrderId` and `Username` values in the **OrderDetails** table.</span></span> <span data-ttu-id="9a9d3-483">이러한 값과 일치 하는 `OrderId` 및 `Username` 에 포함 된 값의 **Orders** 테이블입니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-483">Note that these values match the `OrderId` and `Username` values included in the **Orders** table.</span></span>
11. <span data-ttu-id="9a9d3-484">닫기는 **OrderDetails** 테이블 창.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-484">Close the **OrderDetails** table window.</span></span>
12. <span data-ttu-id="9a9d3-485">Wingtip Toys 데이터베이스 파일을 마우스 오른쪽 단추로 클릭 (*Wingtiptoys.mdf*)을 선택 하 고 **연결 닫기**합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-485">Right-click the Wingtip Toys database file (*Wingtiptoys.mdf*) and select **Close Connection**.</span></span>
13. <span data-ttu-id="9a9d3-486">표시 되지 않으면는 **솔루션 탐색기** 창 클릭 **솔루션 탐색기** 맨 아래에 **서버 탐색기** 표시 하기 위해 창 고 **솔루션 탐색기**  다시 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-486">If you do not see the **Solution Explorer** window, click **Solution Explorer** at the bottom of the **Server Explorer** window to show the **Solution Explorer** again.</span></span>

## <a name="summary"></a><span data-ttu-id="9a9d3-487">요약</span><span class="sxs-lookup"><span data-stu-id="9a9d3-487">Summary</span></span>

<span data-ttu-id="9a9d3-488">이 자습서에서는 제품의 구매를 추적 하는 순서와 주문 정보 스키마를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-488">In this tutorial you added order and order detail schemas to track the purchase of products.</span></span> <span data-ttu-id="9a9d3-489">또한 Wingtip Toys 샘플 응용 프로그램에 PayPal 기능을 통합 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-489">You also integrated PayPal functionality into the Wingtip Toys sample application.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="9a9d3-490">추가 자료</span><span class="sxs-lookup"><span data-stu-id="9a9d3-490">Additional Resources</span></span>

<span data-ttu-id="9a9d3-491">[ASP.NET 구성 개요](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span><span class="sxs-lookup"><span data-stu-id="9a9d3-491">[ASP.NET Configuration Overview](https://msdn.microsoft.com/library/ms178683(v=vs.100).aspx)</span></span>  
[<span data-ttu-id="9a9d3-492">Azure 앱 서비스에 멤버 자격, OAuth, SQL 데이터베이스와 보안 ASP.NET Web Forms 응용 프로그램 배포</span><span class="sxs-lookup"><span data-stu-id="9a9d3-492">Deploy a Secure ASP.NET Web Forms App with Membership, OAuth, and SQL Database to Azure App Service</span></span>](https://azure.microsoft.com/documentation/articles/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/)  
[<span data-ttu-id="9a9d3-493">Microsoft Azure-무료 평가판</span><span class="sxs-lookup"><span data-stu-id="9a9d3-493">Microsoft Azure - Free Trial</span></span>](https://azure.microsoft.com/pricing/free-trial/)

## <a name="disclaimer"></a><span data-ttu-id="9a9d3-494">고지 사항</span><span class="sxs-lookup"><span data-stu-id="9a9d3-494">Disclaimer</span></span>

<span data-ttu-id="9a9d3-495">이 자습서는 샘플 코드를 포함합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-495">This tutorial contains sample code.</span></span> <span data-ttu-id="9a9d3-496">이러한 샘플 코드는 어떤 종류의 보증도 없이 "있는 그대로" 제공 됩니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-496">Such sample code is provided "as is" without warranty of any kind.</span></span> <span data-ttu-id="9a9d3-497">따라서 Microsoft 정확도, 무결성 또는 샘플 코드의 품질을 보장 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-497">Accordingly, Microsoft does not guarantee the accuracy, integrity, or quality of the sample code.</span></span> <span data-ttu-id="9a9d3-498">위험은 사용자가 샘플 코드를 사용 하도록 동의 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-498">You agree to use the sample code at your own risk.</span></span> <span data-ttu-id="9a9d3-499">어떤 경우에는 Microsoft 책임을 지지 않으며를 어떤 방식으로든에서 콘텐츠, 오류 또는 모든 샘플 코드, 콘텐츠, 또는 모든 손실이 나 손상의 모든 샘플 코드 사용으로 인해 발생 하는 어떤 종류의 누락에 제한 되지 않음 모든 샘플 코드에 대 한 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-499">Under no circumstances will Microsoft be liable to you in any way for any sample code, content, including but not limited to, any errors or omissions in any sample code, content, or any loss or damage of any kind incurred as a result of the use of any sample code.</span></span> <span data-ttu-id="9a9d3-500">허 여 메시지가 표시 되며 면책 하 고 저장 한 입지 Microsoft에서 및 모든 손실, 손실, 부상 또는 손상 종류 포함 하 여, 제한 되지 않고 여 occasioned 또는 자료를 게시 하면는에서 발생 하는 클레임에 대해 동의 허 여 않으므로 전송 하 고 사용 하거나 그 안에 의견에 제한 되지 않음 등을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="9a9d3-500">You are hereby notified and do hereby agree to indemnify, save and hold Microsoft harmless from and against any and all loss, claims of loss, injury or damage of any kind including, without limitation, those occasioned by or arising from material that you post, transmit, use or rely on including, but not limited to, the views expressed therein.</span></span>

> [!div class="step-by-step"]
> <span data-ttu-id="9a9d3-501">[이전](shopping-cart.md)
> [다음](membership-and-administration.md)</span><span class="sxs-lookup"><span data-stu-id="9a9d3-501">[Previous](shopping-cart.md)
