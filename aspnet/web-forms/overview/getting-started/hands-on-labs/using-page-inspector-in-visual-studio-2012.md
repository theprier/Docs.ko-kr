...
9. <span data-ttu-id="c7d49-189">HTML 태그에서 Koala 링크를 사용 하 여 목록 항목을 찾아 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-189">In the HTML markup, locate the list item with the Koala link and select it.</span></span>
    ...
    <span data-ttu-id="c7d49-190">코드를 선택 하면 해당 출력을 자동으로 강조 표시 하는 브라우저에서 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-190">Notice that when you select the code, the corresponding output is automatically highlighted in the browser.</span></span> <span data-ttu-id="c7d49-191">이 기능은 HTML 블록을 페이지에 렌더링 되는 방법을 참조 하는 데 유용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-191">This feature is useful to see how an HTML block is rendered on the page.</span></span>
...
<span data-ttu-id="c7d49-240">이 태스크에서는 페이지 검사기 사용 및 사진 갤러리 응용 프로그램 몇 가지 문제를 해결 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-240">In this task, you will use the Page inspector and fix some issues the Photo Gallery application.</span></span>
...
6. <span data-ttu-id="c7d49-322">페이지 검사기 브라우저에서 페이지 요소 위로 마우스를 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-322">In the Page Inspector browser, move the mouse over the page elements.</span></span> <span data-ttu-id="c7d49-323">렌더링된 된 페이지 부분 위로 마우스 포인터를 이동 하는 동안 요소 형식이 표시 되 고 Visual Studio 편집기에서 해당 소스 태그나 코드가 강조 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-323">While you move the mouse pointer over any part of the rendered page, the element type is displayed and the corresponding source markup or code is highlighted in the Visual Studio editor.</span></span>
    ...
    <span data-ttu-id="c7d49-324">![실행 중인 검사 모드](using-page-inspector-in-visual-studio-2012/_static/image28.png "실행 중인 검사 모드")</span><span class="sxs-lookup"><span data-stu-id="c7d49-324">![Inspection mode in action](using-page-inspector-in-visual-studio-2012/_static/image28.png "Inspection mode in action")</span></span>
...
12. <span data-ttu-id="c7d49-348">스타일 창에서 찾을 합니다 **배경 이미지** 아래에 있는 항목을 **.main 콘텐츠** 그룹.</span><span class="sxs-lookup"><span data-stu-id="c7d49-348">In the Styles pane, locate the **background-image** item under the **.main-content** group.</span></span> <span data-ttu-id="c7d49-349">**선택 취소** 는 **배경 이미지** 를 살펴봅니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-349">**Uncheck** the **background-image** and see what happens.</span></span> <span data-ttu-id="c7d49-350">브라우저는 변경 내용의 즉시 반영 됩니다 하 원 숨겨지는지를 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-350">You will notice that the browser will reflect the changes immediately and the circle is hidden.</span></span>
    ...
    <span data-ttu-id="c7d49-354">*설정 및 CSS 스타일을 사용 하지 않도록 설정*</span><span class="sxs-lookup"><span data-stu-id="c7d49-354">*Enabling and disabling CSS styles*</span></span>
...
<span data-ttu-id="c7d49-385">더 실제 wysiwyg (위지윅)에서 ASP.NET WebForms, 일부 스타일 문제를 실행 또는 배포 후 이후 단계에서 검색 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-385">As there is no real WYSIWYG in ASP.NET WebForms, some style issues are detected on a later stage, after running or deploying.</span></span> <span data-ttu-id="c7d49-386">이제 페이지 검사기를 사용 하 여 솔루션을 실행 하지 않고 모든 페이지를 미리 보려면는 것이 같습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-386">Now, with Page Inspector, it is possible to preview any page without running the solution.</span></span>

<span data-ttu-id="c7d49-387">이 태스크에서는 사진 갤러리 응용 프로그램의 몇 가지 문제를 해결 하기 위한 페이지 검사기를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-387">In this task, you will use the Page inspector for fixing some issues of the Photo Gallery application.</span></span> <span data-ttu-id="c7d49-388">다음 단계에서 검색 한 신속 하 게 헤더에 몇 가지 간단한 스타일 문제를 해결 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-388">In the following steps, you will detect and quickly fix some simple styling issue in the header.</span></span>

1. <span data-ttu-id="c7d49-389">페이지 검사를 사용 하 여 찾습니다 합니다 **등록** 하며 **로그인** 헤더의 왼쪽에 있는 링크입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-389">Using Page Inspection, locate the **Register** and the **Log In** links at the left side of the header.</span></span>

    <span data-ttu-id="c7d49-390">링크를 오른쪽에 예상된 된 위치에 표시 되지 않도록 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-390">Notice that the link is not displayed at the expected place on the right.</span></span> <span data-ttu-id="c7d49-391">이제 오른쪽에 대 한 링크를 정렬 하 고 적절 하 게 스타일을 다시 적용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-391">You will now align the link to the right and restyle it accordingly.</span></span>

    <span data-ttu-id="c7d49-392">![왼쪽에 배치 하는 링크에 로그인](using-page-inspector-in-visual-studio-2012/_static/image38.png "왼쪽에 배치 하는 링크에 로그인")</span><span class="sxs-lookup"><span data-stu-id="c7d49-392">![Log in link positioned on the left](using-page-inspector-in-visual-studio-2012/_static/image38.png "Log in link positioned on the left")</span></span>

    <span data-ttu-id="c7d49-393">*로그인 링크는 왼쪽에 배치*</span><span class="sxs-lookup"><span data-stu-id="c7d49-393">*Log In link positioned on the left*</span></span>
2. <span data-ttu-id="c7d49-394">선택한 검사 모드를 설정/해제를 사용 하 여 해당 코드를 열고 로그인 링크를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-394">With Toggle Inspection Mode selected, select the Log In link to open its code.</span></span>

    <span data-ttu-id="c7d49-395">링크 소스 코드에 있는 알림 합니다 **Site.Master** 파일 위치는 Default.aspx 페이지의 첫 번째 위치에서 볼 수 있습니다 하지; 올바른 소스 파일에서 직접 전환할 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-395">Notice that the link source code is located in the **Site.Master** file, not in the Default.aspx page which is the place you might look in first place; you have been placed directly in the correct source file.</span></span>
3. <span data-ttu-id="c7d49-396">에 **스타일** 탭을 찾아 클릭 합니다  **&lt;섹션&gt; #login** 항목 이러한 링크의 HTML 컨테이너입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-396">In the **Styles** tab, locate and click the **&lt;section&gt; #login** item, which is the HTML container for these links.</span></span>

    <span data-ttu-id="c7d49-397">있음을 합니다 **#login** 스타일에 자동으로 위치한 **Site.css** 클릭 한 후입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-397">Notice that the **#login** style is automatically located in **Site.css** after you click.</span></span> <span data-ttu-id="c7d49-398">또한 코드는 이제 강조 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-398">Moreover, the code is now highlighted.</span></span>

    <span data-ttu-id="c7d49-399">![CSS 스타일을 선택 하면](using-page-inspector-in-visual-studio-2012/_static/image39.png "CSS 스타일을 선택 합니다.")</span><span class="sxs-lookup"><span data-stu-id="c7d49-399">![Selecting the CSS styles](using-page-inspector-in-visual-studio-2012/_static/image39.png "Selecting the CSS styles")</span></span>

    <span data-ttu-id="c7d49-400">*CSS 스타일을 선택합니다.*</span><span class="sxs-lookup"><span data-stu-id="c7d49-400">*Selecting the CSS styles*</span></span>
4. <span data-ttu-id="c7d49-401">주석 처리를 제거 합니다 **텍스트 맞춤** 을 열고 닫는 문자를 제거 하 여 강조 표시 된 코드에서 특성 및 저장 합니다 **Site.css** 파일입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-401">Uncomment the **text-align** attribute in the highlighted code by removing the opening and closing characters and save the **Site.css** file.</span></span>

    <span data-ttu-id="c7d49-402">페이지 검사기는 현재 페이지를 구성 하는 모든 다른 파일을 인식 하 고 이러한 파일 중 하나를 변경 하는 경우 검색할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-402">Page Inspector is aware of all the different files that compose the current page, and it can detect when any of these files change.</span></span> <span data-ttu-id="c7d49-403">사용자에 게 경고 때마다 브라우저에서 현재 페이지 원본 파일과 함께 동기화 되지 않았습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-403">It alerts you whenever the current page in browser is not in sync with the source files.</span></span>
5. <span data-ttu-id="c7d49-404">페이지 검사기 브라우저에서 변경 내용을 저장 하 고 페이지를 다시 로드 주소 표시줄 아래에 있는 막대를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-404">In the Page Inspector browser, click the bar located below the address bar to save the changes and reload the page.</span></span>

    ![Reloadingthepage](using-page-inspector-in-visual-studio-2012/_static/image40.png)

    <span data-ttu-id="c7d49-406">*페이지를 다시 로드*</span><span class="sxs-lookup"><span data-stu-id="c7d49-406">*Reloading the page*</span></span>

    <span data-ttu-id="c7d49-407">오른쪽에서 링크 됩니다 있지만 여전히 글머리 기호 목록 처럼 보입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-407">The links are now at the right, but they still look like a bulleted list.</span></span> <span data-ttu-id="c7d49-408">이제 글머리 기호 제거 및 링크를 가로로 정렬 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-408">Now, you will remove the bullets and align the links horizontally.</span></span>

    ![업데이트 된 페이지](using-page-inspector-in-visual-studio-2012/_static/image41.png)

    <span data-ttu-id="c7d49-410">*업데이트 된 페이지*</span><span class="sxs-lookup"><span data-stu-id="c7d49-410">*Updated page*</span></span>
6. <span data-ttu-id="c7d49-411">중 하나를 선택 하는 검사 모드를 사용 하 여 합니다 **&lt;li&gt;** 포함 된 항목을 &quot;등록&quot; 및 &quot;로그인&quot; 링크 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-411">Using the inspection mode, select any of the **&lt;li&gt;** items that contain the &quot;Register&quot; and &quot;Log in&quot; links.</span></span> <span data-ttu-id="c7d49-412">클릭 합니다  **&lt;섹션&gt; #login** 액세스 항목 **Styles.css** 코드입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-412">Then, click the **&lt;section&gt; #login** item to access **Styles.css** code.</span></span>

    <span data-ttu-id="c7d49-413">![스타일 찾기](using-page-inspector-in-visual-studio-2012/_static/image42.png "스타일 찾기")</span><span class="sxs-lookup"><span data-stu-id="c7d49-413">![Finding the style](using-page-inspector-in-visual-studio-2012/_static/image42.png "Finding the style")</span></span>

    <span data-ttu-id="c7d49-414">*스타일 찾기*</span><span class="sxs-lookup"><span data-stu-id="c7d49-414">*Finding the style*</span></span>
7. <span data-ttu-id="c7d49-415">**Style.css**에 대 한 코드를 주석 처리 제거 **#login li** 항목입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-415">In **Style.css**, uncomment the code for **#login li** items.</span></span> <span data-ttu-id="c7d49-416">추가할 스타일 글머리 숨기고 항목 가로로 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-416">The style you are adding will hide the bullet and display the items horizontally.</span></span>

    <span data-ttu-id="c7d49-417">![로그인 링크를 재지정](using-page-inspector-in-visual-studio-2012/_static/image43.png "로그인 링크를 재지정")</span><span class="sxs-lookup"><span data-stu-id="c7d49-417">![Restyling the login links](using-page-inspector-in-visual-studio-2012/_static/image43.png "Restyling the login links")</span></span>

    <span data-ttu-id="c7d49-418">*로그인 링크를 재지정*</span><span class="sxs-lookup"><span data-stu-id="c7d49-418">*Restyling the login links*</span></span>
8. <span data-ttu-id="c7d49-419">저장할 **Style.css** 파일 및 페이지를 다시 로드 하는 주소 아래에 있는 표시줄에서 한 번 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-419">Save **Style.css** file and click once on the bar located below the address to reload the page.</span></span> <span data-ttu-id="c7d49-420">링크를 올바르게 표시 되는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-420">Notice that the links appear correctly.</span></span>

    <span data-ttu-id="c7d49-421">![링크는 왼쪽에서 오른쪽으로 정렬](using-page-inspector-in-visual-studio-2012/_static/image44.png "링크는 왼쪽에서 오른쪽으로 정렬")</span><span class="sxs-lookup"><span data-stu-id="c7d49-421">![Links aligned to the right side](using-page-inspector-in-visual-studio-2012/_static/image44.png "Links aligned to the right side")</span></span>

    <span data-ttu-id="c7d49-422">*왼쪽에서 오른쪽으로 정렬 하는 링크*</span><span class="sxs-lookup"><span data-stu-id="c7d49-422">*Links aligned to the right side*</span></span>
9. <span data-ttu-id="c7d49-423">마지막으로 머리글 제목을 변경 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-423">Finally, you will change the header title.</span></span> <span data-ttu-id="c7d49-424">에 대 한 검색 하는 대신는 '**사용자 로고는 여기 '** 검사 모드는 텍스트를 클릭 하 고 생성 하는 소스 코드를 가져오면를 사용 하는 모든 파일에 텍스트입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-424">Instead of searching for the '**your logo here'** text in all the files, use the inspection mode to click the text and get to source code that generates it.</span></span>

    <span data-ttu-id="c7d49-425">![사이트 제목 찾기](using-page-inspector-in-visual-studio-2012/_static/image45.png "사이트 제목 찾기")</span><span class="sxs-lookup"><span data-stu-id="c7d49-425">![Finding the site title](using-page-inspector-in-visual-studio-2012/_static/image45.png "Finding the site title")</span></span>

    <span data-ttu-id="c7d49-426">*사이트 제목 찾기*</span><span class="sxs-lookup"><span data-stu-id="c7d49-426">*Finding the site title*</span></span>
10. <span data-ttu-id="c7d49-427">에 이제 **Site.Master**, 대체는 '**사용자 로고는 여기**'텍스트'**사진 갤러리**'.</span><span class="sxs-lookup"><span data-stu-id="c7d49-427">Now you are in **Site.Master**, replace the '**your logo here**' text with '**Photo Gallery**'.</span></span> <span data-ttu-id="c7d49-428">저장 하 고 페이지 검사기 브라우저를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-428">Save and update the Page Inspector browser.</span></span>

    <span data-ttu-id="c7d49-429">![사진 갤러리 페이지 업데이트](using-page-inspector-in-visual-studio-2012/_static/image46.png "사진 갤러리 페이지 업데이트")</span><span class="sxs-lookup"><span data-stu-id="c7d49-429">![Photo Gallery page updated](using-page-inspector-in-visual-studio-2012/_static/image46.png "Photo Gallery page updated")</span></span>

    <span data-ttu-id="c7d49-430">*사진 갤러리 페이지 업데이트*</span><span class="sxs-lookup"><span data-stu-id="c7d49-430">*Photo Gallery page updated*</span></span>
11. <span data-ttu-id="c7d49-431">마지막으로 누릅니다 **F5** 체크 아웃 변경 내용이 예상 대로 작동 하는 모든 앱을 실행 하려면.</span><span class="sxs-lookup"><span data-stu-id="c7d49-431">Finally press **F5** to run the app the check out all the changes work as expected.</span></span>
...
<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="c7d49-432">요약</span><span class="sxs-lookup"><span data-stu-id="c7d49-432">Summary</span></span>

<span data-ttu-id="c7d49-433">이 실습을 완료 하면 페이지 검사기를 사용 하 여 웹 응용 프로그램을 다시 빌드하고 브라우저에서 웹 사이트를 실행 하지 않고도 미리 하는 방법을 배웠습니다 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-433">By completing this Hands-On Lab, you have learnt how to use Page Inspector to preview your Web application without having to rebuild and run the Web site in a browser.</span></span> <span data-ttu-id="c7d49-434">또한 신속 하 게 찾기 및 소스 코드에 렌더링된 된 출력에서 직접 액세스 하 여 버그를 수정 하는 방법에 배운 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-434">In addition, you have learnt how to quickly find and fix bugs by accessing directly from the rendered output to the source code.</span></span>

<a id="AppendixA"></a>

<a id="Appendix_A_Installing_Visual_Studio_Express_2012_for_Web"></a>
## <a name="appendix-a-installing-visual-studio-express-2012-for-web"></a><span data-ttu-id="c7d49-435">부록 a: 설치 Visual Studio Express 2012 for Web</span><span class="sxs-lookup"><span data-stu-id="c7d49-435">Appendix A: Installing Visual Studio Express 2012 for Web</span></span>

<span data-ttu-id="c7d49-436">설치할 수 있습니다 **Microsoft Visual Studio Express 2012 for Web** 또는 다른 &quot;Express&quot; 사용 하 여 버전을 **[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span><span class="sxs-lookup"><span data-stu-id="c7d49-436">You can install **Microsoft Visual Studio Express 2012 for Web** or another &quot;Express&quot; version using the **[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span></span> <span data-ttu-id="c7d49-437">다음 지침을 설치 하는 데 필요한 단계를 안내 *Visual studio Express 2012 for Web* 사용 하 여 *Microsoft Web Platform Installer*합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-437">The following instructions guide you through the steps required to install *Visual studio Express 2012 for Web* using *Microsoft Web Platform Installer*.</span></span>

1. <span data-ttu-id="c7d49-438">로 이동 [ [ https://go.microsoft.com/?linkid=9810169 ](https://go.microsoft.com/?linkid=9810169) ](https://go.microsoft.com/?linkid=9810169)합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-438">Go to [[https://go.microsoft.com/?linkid=9810169](https://go.microsoft.com/?linkid=9810169)](https://go.microsoft.com/?linkid=9810169).</span></span> <span data-ttu-id="c7d49-439">또는, 이미 설치한 경우 웹 플랫폼 설치 관리자를 열 수 있습니다 하 고 제품에 대 한 검색 &quot; <em>Visual Studio Express 2012 for Windows Azure SDK를 사용 하 여 Web</em>&quot;합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-439">Alternatively, if you already have installed Web Platform Installer, you can open it and search for the product &quot;<em>Visual Studio Express 2012 for Web with Windows Azure SDK</em>&quot;.</span></span>
2. <span data-ttu-id="c7d49-440">클릭할 **지금 설치**합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-440">Click on **Install Now**.</span></span> <span data-ttu-id="c7d49-441">없는 경우 **웹 플랫폼 설치 관리자** 를 다운로드 하 여 앱을 먼저 설치 리디렉션됩니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-441">If you do not have **Web Platform Installer** you will be redirected to download and install it first.</span></span>
3. <span data-ttu-id="c7d49-442">한 번 **웹 플랫폼 설치 관리자** 열려 있는 경우 클릭 **설치** 는 설치를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-442">Once **Web Platform Installer** is open, click **Install** to start the setup.</span></span>

    <span data-ttu-id="c7d49-443">![Visual Studio Express를 설치](using-page-inspector-in-visual-studio-2012/_static/image47.png "Visual Studio Express를 설치 합니다.")</span><span class="sxs-lookup"><span data-stu-id="c7d49-443">![Install Visual Studio Express](using-page-inspector-in-visual-studio-2012/_static/image47.png "Install Visual Studio Express")</span></span>

    <span data-ttu-id="c7d49-444">*Visual Studio Express를 설치 합니다.*</span><span class="sxs-lookup"><span data-stu-id="c7d49-444">*Install Visual Studio Express*</span></span>
4. <span data-ttu-id="c7d49-445">모든 제품의 라이선스 및 용어를 읽고 클릭 **동의** 를 계속 합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-445">Read all the products' licenses and terms and click **I Accept** to continue.</span></span>

    ![사용 조건에 동의](using-page-inspector-in-visual-studio-2012/_static/image48.png)

    <span data-ttu-id="c7d49-447">*사용 조건에 동의*</span><span class="sxs-lookup"><span data-stu-id="c7d49-447">*Accepting the license terms*</span></span>
5. <span data-ttu-id="c7d49-448">다운로드 및 설치 프로세스가 완료 될 때까지 기다립니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-448">Wait until the downloading and installation process completes.</span></span>

    ![설치 진행률](using-page-inspector-in-visual-studio-2012/_static/image49.png)

    <span data-ttu-id="c7d49-450">*설치 진행률*</span><span class="sxs-lookup"><span data-stu-id="c7d49-450">*Installation progress*</span></span>
6. <span data-ttu-id="c7d49-451">설치가 완료 되 면 클릭 **완료**합니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-451">When the installation completes, click **Finish**.</span></span>

    ![설치 완료](using-page-inspector-in-visual-studio-2012/_static/image50.png)

    <span data-ttu-id="c7d49-453">*설치 완료*</span><span class="sxs-lookup"><span data-stu-id="c7d49-453">*Installation completed*</span></span>
7. <span data-ttu-id="c7d49-454">클릭 **종료** 를 웹 플랫폼 설치 관리자를 닫습니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-454">Click **Exit** to close Web Platform Installer.</span></span>
8. <span data-ttu-id="c7d49-455">로 Visual Studio Express for Web을 열려면 합니다 **시작** 화면 및 쓰기를 시작 &quot; **VS Express**&quot;를 클릭 합니다 **VS Express for Web** 타일입니다.</span><span class="sxs-lookup"><span data-stu-id="c7d49-455">To open Visual Studio Express for Web, go to the **Start** screen and start writing &quot;**VS Express**&quot;, then click on the **VS Express for Web** tile.</span></span>

    ![VS Express for Web 타일](using-page-inspector-in-visual-studio-2012/_static/image51.png)

    <span data-ttu-id="c7d49-457">*VS Express for Web 타일*</span><span class="sxs-lookup"><span data-stu-id="c7d49-457">*VS Express for Web tile*</span></span>
