...
**그림 01**: 새 ASP.NET 페이지를 추가 합니다 `Admin` 폴더 ([클릭 하 여 큰 이미지 보기](interacting-with-the-content-page-from-the-master-page-cs/_static/image3.png))


회수에 [ *마스터 페이지에서 제목, 메타 태그 및 기타 HTML 헤더 지정* ](specifying-the-title-meta-tags-and-other-html-headers-in-the-master-page-cs.md) 라는 기본 페이지를 사용자 지정 클래스를 만들었습니다 자습서 `BasePage` 생성 하는 페이지의 제목이 없는 경우 명시적으로 설정 합니다. 로 이동 합니다 `Products.aspx` 페이지의 코드 숨김 클래스에서 파생 되 게 하 고 `BasePage` (대신에서 `System.Web.UI.Page`).
...
**그림 02**: 새 SqlDataSource 컨트롤을 GridView 바인딩할 ([큰 이미지를 보려면 클릭](interacting-with-the-content-page-from-the-master-page-cs/_static/image6.png))
...
**그림 03**: SqlDataSource Northwind 데이터베이스를 사용 하도록 구성 ([큰 이미지를 보려면 클릭](interacting-with-the-content-page-from-the-master-page-cs/_static/image9.png))
...
**그림 05**: 각 제품 및 해당 가격 GridView에 나열 됩니다 ([큰 이미지를 보려면 클릭](interacting-with-the-content-page-from-the-master-page-cs/_static/image15.png))
...
## <a name="step-2-adding-a-double-prices-button-to-the-master-page"></a>2 단계: 마스터 페이지를 두 번 가격 단추 추가
...
**그림 06**: SqlDataSource를 사용 하 여 구성 합니다 `NorthwindConnectionString` ([클릭 하 여 큰 이미지 보기](interacting-with-the-content-page-from-the-master-page-cs/_static/image18.png))
...
**그림 07**: 설정 SqlDataSource `UpdateCommand` 속성 ([클릭 하 여 큰 이미지 보기](interacting-with-the-content-page-from-the-master-page-cs/_static/image21.png))
...
## <a name="step-3-raising-an-event-when-the-prices-are-doubled"></a>3 단계: 두 배가 되므로 이벤트 경우의 가격을 발생 합니다.
...
*게시자* 이벤트를 제공 하는 개체에 대 한 참조가 *eventName*, 및 *methodName* 에 해당 하는 시그니처를 포함 하는 구독자에 정의 된 이벤트 처리기의 이름 합니다 *eventDelegate*합니다. 즉, 이벤트 대리자 인지 `EventHandler`, 한 다음 *methodName* 값을 반환 하지 않는 두 개의 입력 형식의 매개 변수를 허용 하는 구독자에 있는 메서드의 이름 이어야 합니다 `Object` 고 `EventArgs`, 각각.
...
**그림 08**: 초기 가격 값 ([큰 이미지를 보려면 클릭](interacting-with-the-content-page-from-the-master-page-cs/_static/image24.png))
...
**그림 09**: The Just-Doubled 가격을 Gridview에 표시 됩니다 ([큰 이미지를 보려면 클릭](interacting-with-the-content-page-from-the-master-page-cs/_static/image27.png))
