...
ms.locfileid: "37400641"
---
<a name="displaying-summary-information-in-the-gridviews-footer-vb"></a>GridView의 바닥글 (VB)에서 요약 정보를 표시합니다.
====================
...
**그림 1**: GridView의 바닥글 행에 요약 정보가 표시 됩니다 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image3.png))
...
**그림 2**: 새 ObjectDataSource 라는 추가 `CategoriesDataSource` ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image6.png))
...
**그림 3**: 호출 하는 ObjectDataSource가 합니다 `CategoriesBLL` 클래스의 `GetCategories()` 메서드 ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image9.png))
...
**그림 4**: 사용 하 여는 `CategoryName` 및 `CategoryID` 로 필드를 `Text` 및 `Value` 에 대 한 합니다 `ListItem` s, 각각 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image12.png))
...
**그림 5**: 확인란을 사용 하도록 설정 AutoPostBack DropDownList의 스마트 태그 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image15.png))
...
**그림 6**: 새 ObjectDataSource 라는 추가 `ProductsInCategoryDataSource` ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image18.png))
...
**그림 7**: ObjectDataSource 호출 있어야 합니다 `GetProductsByCategoryID(categoryID)` 메서드 ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image21.png))
...
**그림 8**: 가져오기 합니다 *`categoryID`* 선택한 범주 드롭다운 목록에서 매개 변수 값 ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image24.png))
...
**그림 9**: 가져오기 합니다 *`categoryID`* 선택한 범주 드롭다운 목록에서 매개 변수 값 ([클릭 하 여 큰 이미지 보기](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image27.png))
...
**그림 11**: GridView 이제 바닥글 행이 포함 됩니다 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image33.png))
...
**그림 12**: GridView의 바닥글 행에는 이제 빨강 배경색에 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image36.png))
...
만들기는 `RowDataBound` 를 선택 하 GridView 디자이너에서 속성 창에서 번개 모양 아이콘을 클릭 하 고 두 번 클릭 하면 GridView에 대 한 이벤트 처리기는 `RowDataBound` 이벤트입니다. 또는 ASP.NET 코드 숨김 클래스 파일의 맨 위에 있는 드롭다운 목록에서 GridView 및 해당 RowDataBound 이벤트를 선택할 수 있습니다. 라는 새 이벤트 처리기를 만들어집니다 `ProductsInCategory_RowDataBound` 에 `SummaryDataInFooter.aspx` 페이지의 코드 숨김 클래스입니다.
...
- `_totalNonNullUnitPriceCount`를 형식 `Integer`
- `_totalUnitsInStock`를 형식 `Integer`
- `_totalUnitsOnOrder`를 형식 `Integer`
...
**그림 13**: GridView의 바닥글 행에는 이제 빨강 배경색에 ([큰 이미지를 보려면 클릭](displaying-summary-information-in-the-gridview-s-footer-vb/_static/image39.png))
