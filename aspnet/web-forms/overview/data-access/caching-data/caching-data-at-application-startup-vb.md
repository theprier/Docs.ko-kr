...
ms.locfileid: "41829767"
---
<a name="caching-data-at-application-startup-vb"></a>응용 프로그램 시작 (VB) 시 데이터 캐싱
====================
...
**그림 2**: 만들기 합니다 `StaticCache.vb` 클래스는 `CL` 폴더
...
**그림 3**: 추가 된 `Global.asax` 웹 응용 프로그램 루트 디렉터리에 파일 ([클릭 하 여 큰 이미지 보기](caching-data-at-application-startup-vb/_static/image5.png))
...
**그림 4**: 확인에 중단점을 사용 하는 합니다 `Application_Start` 이벤트 처리기가 실행 중인 ([전체 크기 이미지를 보려면 클릭](caching-data-at-application-startup-vb/_static/image8.png))
...
**그림 5**: ObjectDataSource 사용 하도록 구성 된 `StaticCache` 클래스 ([클릭 하 여 큰 이미지 보기](caching-data-at-application-startup-vb/_static/image11.png))
...
**그림 6**: 사용 된 `GetSuppliers()` 캐시 공급자 데이터를 검색 하는 방법 ([전체 크기 이미지를 보려면 클릭](caching-data-at-application-startup-vb/_static/image14.png))
...
**그림 7**: 캐시 된 공급자 데이터를 GridView에 표시 됩니다 ([큰 이미지를 보려면 클릭](caching-data-at-application-startup-vb/_static/image17.png))
