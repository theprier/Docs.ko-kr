...
<span data-ttu-id="a87b2-129">일반적인 @ Page 지시문을 다음과 같이 표시 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-129">A typical @ Page directive might look like this:</span></span>
...
## <a name="asynctimeout"></a><span data-ttu-id="a87b2-143">Asynctimeout은</span><span class="sxs-lookup"><span data-stu-id="a87b2-143">AsyncTimeout</span></span>
...
## <a name="enableeventvalidation"></a><span data-ttu-id="a87b2-160">EnableEventValidation</span><span class="sxs-lookup"><span data-stu-id="a87b2-160">EnableEventValidation</span></span>
...
<span data-ttu-id="a87b2-220">이 속성 포스트백 및 콜백 이벤트에 대 한 이벤트 유효성 검사를 활성화 하는 지 여부를 제어 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-220">This property controls whether or not event validation is enabled for postback and callback events.</span></span> <span data-ttu-id="a87b2-221">사용 하도록 설정 하면 원래 렌더링 되는 서버 컨트롤에서 발생 될 수 있도록 인수에 다시 게시 또는 콜백 이벤트 확인 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-221">When enabled, arguments to postback or callback events are verified to ensure that they originated from the server control that originally rendered them.</span></span>
...
## <a name="iscallback"></a><span data-ttu-id="a87b2-235">IsCallback</span><span class="sxs-lookup"><span data-stu-id="a87b2-235">IsCallback</span></span>
...
## <a name="masterpagefile"></a><span data-ttu-id="a87b2-249">MasterPageFile</span><span class="sxs-lookup"><span data-stu-id="a87b2-249">MasterPageFile</span></span>
...
<span data-ttu-id="a87b2-263">이 속성 페이지에 적용 되는 스타일 시트를 가져오거나 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-263">This property gets or sets the style sheet that is applied to a page.</span></span>
...
<span data-ttu-id="a87b2-277">요청 하는 장치에서 페이지를 렌더링 하는 ControlAdapter에 대 한 참조를 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-277">Returns a reference to the ControlAdapter that renders the page on the device that requested it.</span></span>
...
<span data-ttu-id="a87b2-291">다음 공용 메서드는 ASP.NET 2.0에서 Page 클래스를 처음 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-291">The following public methods are new to the Page class in ASP.NET 2.0.</span></span>
...
<span data-ttu-id="a87b2-305">이 메서드는 페이지 컨트롤 상태를 유지 해야 되도록 ASP.NET을 알려 줍니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-305">This method tells ASP.NET that the pages control state must be persisted.</span></span>
...
## <a name="loadcomplete-new-in-20"></a><span data-ttu-id="a87b2-332">(2.0)의 새로운 LoadComplete</span><span class="sxs-lookup"><span data-stu-id="a87b2-332">LoadComplete (New in 2.0)</span></span>
...
## <a name="cross-page-postback-in-aspnet-20"></a><span data-ttu-id="a87b2-346">ASP.NET 2.0에서에서 페이지 간 포스트백</span><span class="sxs-lookup"><span data-stu-id="a87b2-346">Cross-Page Postback in ASP.NET 2.0</span></span>
...
3. <span data-ttu-id="a87b2-374">디자인 보기에서 Default.aspx를 열고 단추 컨트롤과 TextBox 컨트롤을 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-374">Open the Default.aspx in Design view and add a Button control and a TextBox control.</span></span> 
    ...
    1. <span data-ttu-id="a87b2-375">단추 컨트롤의 ID를 부여 **제출 단추** TextBox 컨트롤의 ID 및 **UserName**합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-375">Give the Button control an ID of **SubmitButton** and the TextBox control an ID of **UserName**.</span></span>
...
<span data-ttu-id="a87b2-393">설정 하는 비동기 방식으로 실행 하는 페이지의 첫 단계는 **비동기** 같이 page 지시문의 특성:</span><span class="sxs-lookup"><span data-stu-id="a87b2-393">The first step in making a page execute asynchronously is to set the **Async** attribute of the page directive like so:</span></span>
...
## <a name="step-1--implement-the-icallbackeventhandler-interface"></a><span data-ttu-id="a87b2-412">1 단계: ICallbackEventHandler 인터페이스를 구현 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-412">Step 1 : Implement the ICallbackEventHandler Interface</span></span>

<span data-ttu-id="a87b2-413">스크립트 콜백에 참여 하 여 페이지 인식 하기 위해 ASP.NET에서 ICallbackEventHandler 인터페이스를 구현 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-413">In order for ASP.NET to recognize your page as participating in a script callback, you must implement the ICallbackEventHandler interface.</span></span> <span data-ttu-id="a87b2-414">코드 숨김 파일에서 이렇게 하려면 다음과 같이 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-414">You can do this in your code-behind file like so:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample10.cs)]

<span data-ttu-id="a87b2-415">또한 이렇게 하려면 하므로 @ Implements 지시문 like를 사용 하 여:</span><span class="sxs-lookup"><span data-stu-id="a87b2-415">You can also do this using the @ Implements directive like so:</span></span>

[!code-aspx[Main](the-asp-net-2-0-page-model/samples/sample11.aspx)]

<span data-ttu-id="a87b2-416">일반적으로 인라인 ASP.NET 코드를 사용 하는 경우에 @ Implements 지시문을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-416">You would typically use the @ Implements directive when using inline ASP.NET code.</span></span>

## <a name="step-2--call-getcallbackeventreference"></a><span data-ttu-id="a87b2-417">2 단계: GetCallbackEventReference 호출</span><span class="sxs-lookup"><span data-stu-id="a87b2-417">Step 2 : Call GetCallbackEventReference</span></span>

<span data-ttu-id="a87b2-418">앞에서 설명한 대로 XMLHttp 호출 WebResource.axd 처리기에 캡슐화 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-418">As mentioned previously, the XMLHttp call is encapsulated in the WebResource.axd handler.</span></span> <span data-ttu-id="a87b2-419">ASP.NET WebForm에 대 한 호출을 추가 합니다 페이지를 렌더링할 때\_DoCallback, WebResource.axd에서 제공 되는 클라이언트 스크립트입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-419">When your page is rendered, ASP.NET will add a call to WebForm\_DoCallback, a client script that is provided by WebResource.axd.</span></span> <span data-ttu-id="a87b2-420">WebForm\_DoCallback 함수 대체는 \_ \_doPostBack 함수는 콜백에 대 한 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-420">The WebForm\_DoCallback function replaces the \_\_doPostBack function for a callback.</span></span> <span data-ttu-id="a87b2-421">에 유의 해야 \_ \_doPostBack 페이지에서 폼을 프로그래밍 방식으로 전송 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-421">Remember that \_\_doPostBack programmatically submits the form on the page.</span></span> <span data-ttu-id="a87b2-422">따라서에서 포스트백을 방지 하려면 콜백 시나리오에서 \_ \_doPostBack 적절 하지 것입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-422">In a callback scenario, you want to prevent a postback, so \_\_doPostBack will not suffice.</span></span>

> [!NOTE]
> <span data-ttu-id="a87b2-423">\_\_doPostBack은 클라이언트 스크립트 콜백 시나리오의 페이지에 계속 렌더링 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-423">\_\_doPostBack is still rendered to the page in a client script callback scenario.</span></span> <span data-ttu-id="a87b2-424">그러나 콜백에 대 한 사용 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-424">However, it's not used for the callback.</span></span>


<span data-ttu-id="a87b2-425">인수는 WebForm\_DoCallback 클라이언트 쪽 함수는 서버 쪽 기능 GetCallbackEventReference 페이지에서 정상적으로 호출을 통해 제공 됩니다\_로드 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-425">The arguments for the WebForm\_DoCallback client-side function are provided via the server-side function GetCallbackEventReference which would normally be called in Page\_Load.</span></span> <span data-ttu-id="a87b2-426">GetCallbackEventReference 호출을 다음과 같이 표시 될 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-426">A typical call to GetCallbackEventReference might look like this:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample12.cs)]

> [!NOTE]
> <span data-ttu-id="a87b2-427">이 경우 cm는 ClientScriptManager의 인스턴스입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-427">In this case, cm is an instance of ClientScriptManager.</span></span> <span data-ttu-id="a87b2-428">이 모듈의 뒷부분에 나오는 ClientScriptManager 클래스를 설명 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-428">The ClientScriptManager class will be covered later in this module.</span></span>


<span data-ttu-id="a87b2-429">GetCallbackEventReference의 몇 가지 오버 로드 된 버전이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-429">There are several overloaded versions of GetCallbackEventReference.</span></span> <span data-ttu-id="a87b2-430">이 경우 인수는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-430">In this case, the arguments are as follows:</span></span>

`this`

<span data-ttu-id="a87b2-431">GetCallbackEventReference 호출 되는 컨트롤에 대 한 참조입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-431">A reference to the control where GetCallbackEventReference is being called.</span></span> <span data-ttu-id="a87b2-432">이 경우,이 페이지 자체입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-432">In this case, it's the page itself.</span></span>

[!code-javascript[Main](the-asp-net-2-0-page-model/samples/sample13.js)]

<span data-ttu-id="a87b2-433">서버 쪽 이벤트를 클라이언트 코드에서 전달 되는 문자열 인수입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-433">A string argument that will be passed from the client-side code to the server-side event.</span></span> <span data-ttu-id="a87b2-434">이 경우의 드롭다운 값을 전달 하는 Im ddlCompany 호출 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-434">In this case, Im passing the value of a dropdown called ddlCompany.</span></span>

`ShowCompanyName`

<span data-ttu-id="a87b2-435">서버 쪽 콜백 이벤트에서 반환 값 (문자열)으로 허용 하는 클라이언트 쪽 함수의 이름.</span><span class="sxs-lookup"><span data-stu-id="a87b2-435">The name of the client-side function that will accept the return value (as string) from the server-side callback event.</span></span> <span data-ttu-id="a87b2-436">이 함수는 서버 쪽 콜백에 성공할 경우에 호출 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-436">This function will only be called when the server-side callback is successful.</span></span> <span data-ttu-id="a87b2-437">따라서 견고성을 위해 오류 발생 시 실행 하는 클라이언트 쪽 함수 이름을 지정 하는 추가 문자열 인수를 사용 하는 GetCallbackEventReference의 오버 로드 된 버전을 사용 하도록 좋습니다 일반적으로 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-437">Therefore, for the sake of robustness, it is generally recommended to use the overloaded version of GetCallbackEventReference that takes an additional string argument specifying the name of a client-side function to execute in the event of an error.</span></span>

`null`

<span data-ttu-id="a87b2-438">서버에는 콜백 전에 시작 된 클라이언트 쪽 함수를 나타내는 문자열입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-438">A string representing a client-side function that it initiated before the callback to the server.</span></span> <span data-ttu-id="a87b2-439">이 경우가 해당 스크립트가 없습니다. 인수가 null입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-439">In this case, there is no such script, so the argument is null.</span></span>

`true`

<span data-ttu-id="a87b2-440">콜백을 비동기적으로 수행 여부를 지정 하는 부울입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-440">A Boolean specifying whether or not to conduct the callback asynchronously.</span></span>

<span data-ttu-id="a87b2-441">WebForm에 대 한 호출\_DoCallback 클라이언트에서 이러한 인수를 전달 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-441">The call to WebForm\_DoCallback on the client will pass these arguments.</span></span> <span data-ttu-id="a87b2-442">따라서 클라이언트에서이 페이지를 렌더링할 때 해당 코드는 같습니다 같이:</span><span class="sxs-lookup"><span data-stu-id="a87b2-442">Therefore, when this page is rendered on the client, that code will look like so:</span></span>

[!code-javascript[Main](the-asp-net-2-0-page-model/samples/sample14.js)]

<span data-ttu-id="a87b2-443">클라이언트에서 함수의 서명이 약간 다르게 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-443">Notice that the signature of the function on the client is a bit different.</span></span> <span data-ttu-id="a87b2-444">클라이언트 쪽 함수는 5 개 문자열 및 부울 값을 전달합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-444">The client-side function passes 5 strings and a Boolean.</span></span> <span data-ttu-id="a87b2-445">서버 쪽 콜백이 발생 하는 오류를 처리 하는 클라이언트 쪽 함수를 포함 하는 추가 문자열 (위의 예제에서 null은)이 표시를 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-445">The additional string (which is null in the above example) contains the client-side function that will handle any errors from the server-side callback.</span></span>

## <a name="step-3--hook-the-client-side-control-event"></a><span data-ttu-id="a87b2-446">3 단계: 클라이언트 쪽 컨트롤 이벤트를 후크</span><span class="sxs-lookup"><span data-stu-id="a87b2-446">Step 3 : Hook the Client-Side Control Event</span></span>

<span data-ttu-id="a87b2-447">위의 GetCallbackEventReference의 반환 값을 문자열 변수에 할당 된를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-447">Notice that the return value of GetCallbackEventReference above was assigned to a string variable.</span></span> <span data-ttu-id="a87b2-448">해당 문자열이 콜백을 시작 하는 컨트롤에 대 한 클라이언트 쪽 이벤트를 후크 하는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-448">That string is used to hook a client-side event for the control that initiates the callback.</span></span> <span data-ttu-id="a87b2-449">이 예제에서는 콜백이 시작 페이지에서 드롭다운 이므로 연결 하고자 하는 경우는 *OnChange* 이벤트입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-449">In this example, the callback is initiated by a dropdown on the page, so I want to hook the *OnChange* event.</span></span>

<span data-ttu-id="a87b2-450">연결할 클라이언트측 이벤트 처리기를 클라이언트 쪽 태그를 다음과 같이 추가:</span><span class="sxs-lookup"><span data-stu-id="a87b2-450">To hook the client-side event, simply add a handler to the client-side markup as follows:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample15.cs)]

<span data-ttu-id="a87b2-451">이전에 설명한 대로 *cbRef* GetCallbackEventReference에 대 한 호출에서 반환 값입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-451">Recall that *cbRef* is the return value from the call to GetCallbackEventReference.</span></span> <span data-ttu-id="a87b2-452">WebForm에 대 한 호출을 포함\_위에 나온 DoCallback 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-452">It contains the call to WebForm\_DoCallback that was shown above.</span></span>

## <a name="step-4--register-the-client-side-script"></a><span data-ttu-id="a87b2-453">4 단계: 클라이언트 쪽 스크립트를 등록 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-453">Step 4 : Register the Client-Side Script</span></span>

<span data-ttu-id="a87b2-454">회수 GetCallbackEventReference에 대 한 호출 클라이언트 쪽 스크립트에 호출 되도록 지정 **ShowCompanyName** 하면 서버 쪽 콜백 성공할 때 실행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-454">Recall that the call to GetCallbackEventReference specified that a client-side script called **ShowCompanyName** would be executed when the server-side callback succeeds.</span></span> <span data-ttu-id="a87b2-455">해당 스크립트 ClientScriptManager 인스턴스를 사용 하 여 페이지에 추가 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-455">That script needs to be added to the page using a ClientScriptManager instance.</span></span> <span data-ttu-id="a87b2-456">(ClientScriptManager 클래스에이 모듈의 뒷부분에 나오는 표에서 수 있습니다.) 해당 같은 지금 수행합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-456">(The ClientScriptManager class will be dicussed later in this module.) You do that like so:</span></span>

[!code-javascript[Main](the-asp-net-2-0-page-model/samples/sample16.js)]

## <a name="step-5--call-the-methods-of-the-icallbackeventhandler-interface"></a><span data-ttu-id="a87b2-457">5 단계: ICallbackEventHandler 인터페이스의 메서드를 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-457">Step 5 : Call the Methods of the ICallbackEventHandler Interface</span></span>

<span data-ttu-id="a87b2-458">ICallbackEventHandler 코드에서 구현 해야 하는 두 개의 메서드를 포함 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-458">The ICallbackEventHandler contains two methods that you need to implement in your code.</span></span> <span data-ttu-id="a87b2-459">이들은 **RaiseCallbackEvent** 및 **GetCallbackEvent**합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-459">They are **RaiseCallbackEvent** and **GetCallbackEvent**.</span></span>

<span data-ttu-id="a87b2-460">**RaiseCallbackEvent** 인수로 문자열을 nothing을 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-460">**RaiseCallbackEvent** takes a string as an argument and returns nothing.</span></span> <span data-ttu-id="a87b2-461">WebForm 클라이언트 호출에서 문자열 인수가 전달 되었습니다\_DoCallback 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-461">The string argument is passed from the client-side call to WebForm\_DoCallback.</span></span> <span data-ttu-id="a87b2-462">이 경우 해당 값은는 *값* 특성 ddlCompany 라는 드롭다운입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-462">In this case, that value is the *value* attribute of the dropdown called ddlCompany.</span></span> <span data-ttu-id="a87b2-463">서버 쪽 코드 RaiseCallbackEvent 메서드에 배치 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-463">Your server-side code should be placed in the RaiseCallbackEvent method.</span></span> <span data-ttu-id="a87b2-464">예를 들어 콜백은 외부 리소스에 대해 WebRequest를 하는 경우 해당 코드 RaiseCallbackEvent에 배치 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-464">For example, if your callback is making a WebRequest against an external resource, that code should be placed in RaiseCallbackEvent.</span></span>

<span data-ttu-id="a87b2-465">**GetCallbackEvent** 을 클라이언트에 콜백 반환을 처리 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-465">**GetCallbackEvent** is responsible for processing the return of the callback to the client.</span></span> <span data-ttu-id="a87b2-466">인수를 받지 않는 하 고 문자열을 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-466">It takes no arguments and returns a string.</span></span> <span data-ttu-id="a87b2-467">반환 되는 문자열 전달 됩니다 인수로 서 클라이언트 쪽 함수에이 예에서 *ShowCompanyName*합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-467">The string that it returns will be passed as an argument to the client-side function, in this case *ShowCompanyName*.</span></span>

<span data-ttu-id="a87b2-468">위의 단계를 완료 후 스크립트 콜백을 수행 하 여 ASP.NET 2.0에서 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-468">Once you have completed the above steps, you are ready to perform a script callback in ASP.NET 2.0.</span></span>


![](the-asp-net-2-0-page-model/_static/image4.png)


[<span data-ttu-id="a87b2-469">전체 화면 비디오 열기</span><span class="sxs-lookup"><span data-stu-id="a87b2-469">Open Full-Screen Video</span></span>](the-asp-net-2-0-page-model/_static/callback1.wmv)


<span data-ttu-id="a87b2-470">ASP.NET에서 콜백 스크립트는 함으로써 XMLHttp 호출을 지 원하는 모든 브라우저에서 지원 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-470">Script callbacks in ASP.NET are supported in any browser that supports making XMLHttp calls.</span></span> <span data-ttu-id="a87b2-471">포함 된 모든 최신 브라우저 사용 하 여 오늘날.</span><span class="sxs-lookup"><span data-stu-id="a87b2-471">That includes all of the modern browsers in use today.</span></span> <span data-ttu-id="a87b2-472">Internet Explorer (예정 된 IE 7 포함) 다른 최신 브라우저 내장 XMLHttp 개체를 사용 하는 동안 XMLHttp ActiveX 개체를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-472">Internet Explorer uses the XMLHttp ActiveX object while other modern browsers (including the upcoming IE 7) use an intrinsic XMLHttp object.</span></span> <span data-ttu-id="a87b2-473">프로그래밍 방식으로 사용할 수 있는 브라우저 콜백을 지 원하는 경우를 결정 하는 **Request.Browser.SupportCallback** 속성입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-473">To programmatically determine if a browser supports callbacks, you can use the **Request.Browser.SupportCallback** property.</span></span> <span data-ttu-id="a87b2-474">이 속성은 반환 **true** 요청 하는 클라이언트 스크립트 콜백을 지 원하는 경우.</span><span class="sxs-lookup"><span data-stu-id="a87b2-474">This property will return **true** if the requesting client supports script callbacks.</span></span>

## <a name="working-with-client-script-in-aspnet-20"></a><span data-ttu-id="a87b2-475">ASP.NET 2.0에서에서 클라이언트 스크립트 작업</span><span class="sxs-lookup"><span data-stu-id="a87b2-475">Working with Client Script in ASP.NET 2.0</span></span>

<span data-ttu-id="a87b2-476">ASP.NET 2.0에서 클라이언트 스크립트 ClientScriptManager 클래스의 용도 통해 관리 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-476">Client scripts in ASP.NET 2.0 are managed via the use of the ClientScriptManager class.</span></span> <span data-ttu-id="a87b2-477">형식과 이름을 사용 하 여 클라이언트 스크립트 ClientScriptManager 클래스는 추적입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-477">The ClientScriptManager class keeps track of client scripts using a type and a name.</span></span> <span data-ttu-id="a87b2-478">이렇게 하면 동일한 스크립트를 두 번 이상 삽입 페이지에 프로그래밍 방식으로 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-478">This prevents the same script from being programmatically inserted on a page more than once.</span></span>

> [!NOTE]
> <span data-ttu-id="a87b2-479">스크립트를 페이지에 성공적으로 등록 된 후 스크립트를 두 번째로 등록 되었을 단순히 후속 동일한 스크립트를 등록 하려고가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-479">After a script has been successfully registered on a page, any subsequent attempt to register the same script will simply result in the script not being registered a second time.</span></span> <span data-ttu-id="a87b2-480">중복 된 스크립트가 추가 되 고 예외가 발생 하지 않으면.</span><span class="sxs-lookup"><span data-stu-id="a87b2-480">No duplicate scripts are added and no exception occurs.</span></span> <span data-ttu-id="a87b2-481">불필요 한 계산을 방지 하려면 두 번 이상 등록 하지 마십시오 하는 스크립트가 이미 등록 되었는지 확인 하는 데 사용할 수 있는 메서드가 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-481">To avoid unnecessary computation, there are methods that you can use to determine if a script is already registered so that you do not attempt to register it more than once.</span></span>


<span data-ttu-id="a87b2-482">ClientScriptManager의 메서드는 모든 현재 ASP.NET 개발자에 게 잘 알고 있어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-482">The methods of the ClientScriptManager should be familiar to all current ASP.NET developers:</span></span>

## <a name="registerclientscriptblock"></a><span data-ttu-id="a87b2-483">RegisterClientScriptBlock</span><span class="sxs-lookup"><span data-stu-id="a87b2-483">RegisterClientScriptBlock</span></span>

<span data-ttu-id="a87b2-484">이 메서드는 렌더링 된 페이지의 맨 위에 스크립트를 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-484">This method adds a script to the top of the rendered page.</span></span> <span data-ttu-id="a87b2-485">클라이언트에서 명시적으로 호출 될 함수를 추가 하는 데 유용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-485">This is useful for adding functions that will be explicitly called on the client.</span></span>

<span data-ttu-id="a87b2-486">이 메서드의 두 오버 로드 된 버전이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-486">There are two overloaded versions of this method.</span></span> <span data-ttu-id="a87b2-487">4 개 인수 중 3 개는 해당 간에 공통적으로 적용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-487">Three of four arguments are common among them.</span></span> <span data-ttu-id="a87b2-488">다음 창이 여기에 포함됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-488">They are:</span></span>

`type (string)`

<span data-ttu-id="a87b2-489">***형식*** 인수는 스크립트에 대 한 유형을 식별 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-489">The ***type*** argument identifies a type for the script.</span></span> <span data-ttu-id="a87b2-490">일반적으로 것이 좋습니다 (이 페이지의 형식을 사용 하려면입니다. 유형에 대 한 GetType()) 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-490">It is generally a good idea to use the page's type (this.GetType()) for the type.</span></span>

`key (string)`

<span data-ttu-id="a87b2-491">***키*** 인수는 스크립트에 대 한 사용자 정의 키입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-491">The ***key*** argument is a user-defined key for the script.</span></span> <span data-ttu-id="a87b2-492">각 스크립트에 대해 고유 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-492">This should be unique for each script.</span></span> <span data-ttu-id="a87b2-493">동일한 키 및 이미 추가 된 스크립트 유형을 사용 하 여 스크립트를 추가 하려고 하면 추가 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-493">If you attempt to add a script with the same key and type of an already added script, it will not be added.</span></span>

`script (string)`

<span data-ttu-id="a87b2-494">***스크립트*** 인수는 실제 스크립트 추가를 포함 하는 문자열입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-494">The ***script*** argument is a string containing the actual script to add.</span></span> <span data-ttu-id="a87b2-495">스크립트를 만들고 다음를 할당 하려면 StringBuilder에 tostring () 메서드를 사용 하는 StringBuilder를 사용 하는 것이 좋습니다.는 ***스크립트*** 인수입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-495">It's recommended that you use a StringBuilder to create the script and then use the ToString() method on the StringBuilder to assign the ***script*** argument.</span></span>

<span data-ttu-id="a87b2-496">Script 요소를 포함 해야만 세 개의 인수를 받아들이는 오버 로드 된 RegisterClientScriptBlock를 사용 하는 경우 (&lt;스크립트&gt; 및 &lt;/script&gt;) 스크립트에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-496">If you use the overloaded RegisterClientScriptBlock that only takes three arguments, you must include script elements (&lt;script&gt; and &lt;/script&gt;) in your script.</span></span>

<span data-ttu-id="a87b2-497">RegisterClientScriptBlock 네 번째 인수를 받아들이는 오버 로드를 사용 하도록 선택할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-497">You may choose to use the overload of RegisterClientScriptBlock that takes a fourth argument.</span></span> <span data-ttu-id="a87b2-498">네 번째 인수에는 ASP.NET 드립니다 script 요소를 추가할지 여부를 지정 하는 부울입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-498">The fourth argument is a Boolean that specifies whether or not ASP.NET should add script elements for you.</span></span> <span data-ttu-id="a87b2-499">이 인수가 **true**, 스크립트는 스크립트 요소 명시적으로 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-499">If this argument is **true**, your script should not include the script elements explicitly.</span></span>

<span data-ttu-id="a87b2-500">스크립트를 이미 등록 되었는지 확인 하려면 IsClientScriptBlockRegistered 메서드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-500">Use the IsClientScriptBlockRegistered method to determine if a script has already been registered.</span></span> <span data-ttu-id="a87b2-501">이렇게 하면 이미 등록 되어 있는 스크립트를 다시 등록 하는 시도 방지할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-501">This allows you to avoid an attempt to re-register a script that has already been registered.</span></span>

### <a name="registerclientscriptinclude-new-in-20"></a><span data-ttu-id="a87b2-502">(2.0)의 새로운 RegisterClientScriptInclude</span><span class="sxs-lookup"><span data-stu-id="a87b2-502">RegisterClientScriptInclude (New in 2.0)</span></span>

<span data-ttu-id="a87b2-503">RegisterClientScriptInclude 태그 외부 스크립트 파일에 연결 되는 스크립트 블록을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-503">The RegisterClientScriptInclude tag creates a script block that links to an external script file.</span></span> <span data-ttu-id="a87b2-504">두 오버 로드를가지고 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-504">It has two overloads.</span></span> <span data-ttu-id="a87b2-505">하나는 키와 URL을 사용합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-505">One takes a key and a URL.</span></span> <span data-ttu-id="a87b2-506">두 번째 유형을 지정 하는 세 번째 인수를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-506">The second adds a third argument specifying the type.</span></span>

<span data-ttu-id="a87b2-507">예를 들어 다음 코드에서는 스크립트 블록 jsfunctions.js에 연결 되는 응용 프로그램의 스크립트 폴더의 루트에서:</span><span class="sxs-lookup"><span data-stu-id="a87b2-507">For example, the following code generates a script block that links to jsfunctions.js in the root of the scripts folder of the application:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample17.cs)]

<span data-ttu-id="a87b2-508">이 코드는 렌더링 된 페이지에 다음 코드를 생성합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-508">This code produces the following code in the rendered page:</span></span>

[!code-html[Main](the-asp-net-2-0-page-model/samples/sample18.html)]

> [!NOTE]
> <span data-ttu-id="a87b2-509">스크립트 블록은 페이지 아래쪽에 렌더링 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-509">The script block is rendered at the bottom of the page.</span></span>


<span data-ttu-id="a87b2-510">스크립트를 이미 등록 되었는지 확인 하려면 IsClientScriptIncludeRegistered 메서드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-510">Use the IsClientScriptIncludeRegistered method to determine if a script has already been registered.</span></span> <span data-ttu-id="a87b2-511">이렇게 하면 스크립트를 다시 등록 하는 시도 방지할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-511">This allows you to avoid an attempt to re-register a script.</span></span>

## <a name="registerstartupscript"></a><span data-ttu-id="a87b2-512">RegisterStartupScript</span><span class="sxs-lookup"><span data-stu-id="a87b2-512">RegisterStartupScript</span></span>

<span data-ttu-id="a87b2-513">RegisterStartupScript 메서드가 RegisterClientScriptBlock 방법으로 동일한 인수를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-513">The RegisterStartupScript method takes the same arguments as the RegisterClientScriptBlock method.</span></span> <span data-ttu-id="a87b2-514">등록 된 RegisterStartupScript 스크립트 페이지가 로드 하기 전에 실행 OnLoad 클라이언트 쪽 이벤트입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-514">A script registered with RegisterStartupScript executes after the page loads but before the OnLoad client-side event.</span></span> <span data-ttu-id="a87b2-515">1.X 등록 RegisterStartupScript 된 스크립트는 닫는 직전 배치 된 &lt;/f&gt; 등록 된 RegisterClientScriptBlock 스크립트 열기 바로 뒤에 배치 된 동안 태그 &lt;양식&gt; 태그입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-515">In 1.X, scripts registered with RegisterStartupScript were placed just before the closing &lt;/form&gt; tag while scripts registered with RegisterClientScriptBlock were placed immediately after the opening &lt;form&gt; tag.</span></span> <span data-ttu-id="a87b2-516">ASP.NET 2.0에서는 닫는 직전 배치 됩니다 둘 다 &lt;/f&gt; 태그입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-516">In ASP.NET 2.0, both are placed immediately before the closing &lt;/form&gt; tag.</span></span>

> [!NOTE]
> <span data-ttu-id="a87b2-517">RegisterStartupScript와 함수를 등록 하는 경우 클라이언트 코드에서 명시적으로 호출 될 때까지 해당 함수가 실행 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-517">If you register a function with RegisterStartupScript, that function will not execute until you explicitly call it in client-side code.</span></span>


<span data-ttu-id="a87b2-518">스크립트를 이미 등록 되었는지 확인 하는 스크립트를 다시 등록 하려고를 방지 하려면 IsStartupScriptRegistered 메서드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-518">Use the IsStartupScriptRegistered method to determine if a script has already been registered and avoid an attempt to re-register a script.</span></span>

## <a name="other-clientscriptmanager-methods"></a><span data-ttu-id="a87b2-519">다른 ClientScriptManager 메서드</span><span class="sxs-lookup"><span data-stu-id="a87b2-519">Other ClientScriptManager Methods</span></span>

<span data-ttu-id="a87b2-520">다음은 ClientScriptManager 클래스의 다른 유용한 메서드 중 일부입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-520">Here are some of the other useful methods of the ClientScriptManager class.</span></span>

| <span data-ttu-id="a87b2-521">**GetCallbackEventReference**</span><span class="sxs-lookup"><span data-stu-id="a87b2-521">**GetCallbackEventReference**</span></span> | <span data-ttu-id="a87b2-522">이 모듈의 앞부분에 나오는 스크립트 콜백을 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="a87b2-522">See script callbacks earlier in this module.</span></span> |
| --- | --- |
| <span data-ttu-id="a87b2-523">**GetPostBackClientHyperlink**</span><span class="sxs-lookup"><span data-stu-id="a87b2-523">**GetPostBackClientHyperlink**</span></span> | <span data-ttu-id="a87b2-524">JavaScript 참조를 가져옵니다 (javascript:&lt;호출&gt;) 클라이언트 쪽 이벤트에서 다시 게시 하려면 사용할 수 있는 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-524">Gets a JavaScript reference (javascript:&lt;call&gt;) that can be used to post back from a client-side event.</span></span> |
| <span data-ttu-id="a87b2-525">**GetPostBackEventReference**</span><span class="sxs-lookup"><span data-stu-id="a87b2-525">**GetPostBackEventReference**</span></span> | <span data-ttu-id="a87b2-526">클라이언트에서 다시 게시를 시작 하려면 사용할 수 있는 문자열을 가져옵니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-526">Gets a string that can be used to initiate a post back from the client.</span></span> |
| <span data-ttu-id="a87b2-527">**GetWebResourceUrl**</span><span class="sxs-lookup"><span data-stu-id="a87b2-527">**GetWebResourceUrl**</span></span> | <span data-ttu-id="a87b2-528">어셈블리에 포함 된 리소스에 대 한 URL을 반환 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-528">Returns a URL to a resource that is embedded in an assembly.</span></span> <span data-ttu-id="a87b2-529">와 함께에서 사용 해야 **RegisterClientScriptResource**합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-529">Must be used in conjunction with **RegisterClientScriptResource**.</span></span> |
| <span data-ttu-id="a87b2-530">**RegisterClientScriptResource**</span><span class="sxs-lookup"><span data-stu-id="a87b2-530">**RegisterClientScriptResource**</span></span> | <span data-ttu-id="a87b2-531">페이지와 웹 리소스를 등록합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-531">Registers a Web resource with the page.</span></span> <span data-ttu-id="a87b2-532">이들은 새 WebResource.axd 처리기에서 처리 및 어셈블리에 포함 된 리소스입니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-532">These are resources embedded in an assembly and handled by the new WebResource.axd handler.</span></span> |
| <span data-ttu-id="a87b2-533">**RegisterHiddenField**</span><span class="sxs-lookup"><span data-stu-id="a87b2-533">**RegisterHiddenField**</span></span> | <span data-ttu-id="a87b2-534">페이지와 숨겨진된 양식 필드를 등록합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-534">Registers a hidden form field with the page.</span></span> |
| <span data-ttu-id="a87b2-535">**RegisterOnSubmitStatement**</span><span class="sxs-lookup"><span data-stu-id="a87b2-535">**RegisterOnSubmitStatement**</span></span> | <span data-ttu-id="a87b2-536">HTML 폼이 전송 될 때 실행 되는 클라이언트 쪽 코드를 등록 합니다.</span><span class="sxs-lookup"><span data-stu-id="a87b2-536">Registers client-side code that executes when the HTML form is submitted.</span></span> |
