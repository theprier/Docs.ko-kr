...
- [<span data-ttu-id="3a8cf-111">알려진된 문제</span><span class="sxs-lookup"><span data-stu-id="3a8cf-111">Known issues</span></span>](#knownissues)
...
### <a name="input-validation"></a><span data-ttu-id="3a8cf-127">입력된 유효성 검사</span><span class="sxs-lookup"><span data-stu-id="3a8cf-127">Input validation</span></span>
...
### <a name="view-models-recommended"></a><span data-ttu-id="3a8cf-155">권장 모델 보기</span><span class="sxs-lookup"><span data-stu-id="3a8cf-155">View models recommended</span></span>
...
<span data-ttu-id="3a8cf-217">반환 비동기 메서드는 값을 반환할 필요가 없습니다, 하는 경우는 `Task` 형식 대신 `void`합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-217">If an async method doesn't need to return a value, return a `Task` type rather than `void`.</span></span>
...
1. <span data-ttu-id="3a8cf-246">설치 [Visual Studio 2013 또는 Visual Studio 2013 Express for Web](https://www.visualstudio.com/downloads)합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-246">Install [Visual Studio 2013 or Visual Studio 2013 Express for Web](https://www.visualstudio.com/downloads).</span></span>
...
8. <span data-ttu-id="3a8cf-302">이러한 스크립트를 실행 하려면 Windows powershell을 사용 하 여는 [Unblock-file](https://go.microsoft.com/fwlink/p/?linkid=294021) cmdlet.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-302">To allow Windows PowerShell to run these scripts, use the [Unblock-File](https://go.microsoft.com/fwlink/p/?linkid=294021) cmdlet.</span></span> <span data-ttu-id="3a8cf-303">(스크립트 때문에 차단 됩니다 인터넷에서 다운로드 합니다.)</span><span class="sxs-lookup"><span data-stu-id="3a8cf-303">(The scripts are blocked because they were downloaded from the Internet.)</span></span>
    ...
    <span data-ttu-id="3a8cf-305">예를 들어 다음 명령이 실행 되는 `Unblock-File` 현재 디렉터리에서 모든 스크립트에는 cmdlet입니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-305">For example, the following command runs the `Unblock-File` cmdlet on all scripts in the current directory.</span></span>
...
<span data-ttu-id="3a8cf-353">MyFixIt.WorkerRoler\app.config에서 아래 `connectionStrings`의 값을 바꿉니다는 `appdb` SQL 데이터베이스에 대 한 실제 연결 문자열과 연결 문자열입니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-353">In MyFixIt.WorkerRoler\app.config, under `connectionStrings`, replace the value of the `appdb` connection string with the actual connection string for the SQL Database.</span></span> <span data-ttu-id="3a8cf-354">포털에서 연결 문자열을 가져올 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-354">You can get the connection string from the portal.</span></span> <span data-ttu-id="3a8cf-355">포털에서 클릭 **SQL 데이터베이스** - **appdb** - **ADO.Net, ODBC, PHP 및 JDBC에 대 한 보기 SQL 데이터베이스 연결 문자열**합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-355">In the portal, click **SQL Databases** - **appdb** - **View SQL Database connection strings for ADO .Net, ODBC, PHP, and JDBC**.</span></span> <span data-ttu-id="3a8cf-356">ADO.NET 연결 문자열을 복사 하 고 값을 app.config 파일에 붙여넣습니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-356">Copy the ADO.NET connection string and paste the value into the app.config file.</span></span> <span data-ttu-id="3a8cf-357">대체 "{프로그램\_암호\_여기}" 데이터베이스 암호를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-357">Replace "{your\_password\_here}" with your database password.</span></span> <span data-ttu-id="3a8cf-358">(에 데이터베이스 암호를 지정한 MVC 앱을 배포 하는 스크립트를 사용 하는 것으로 가정 합니다는 `SqlDatabasePassword` 매개 변수를 스크립팅 합니다.)</span><span class="sxs-lookup"><span data-stu-id="3a8cf-358">(Assuming you used the scripts to deploy the MVC app, you specified the database password in the `SqlDatabasePassword` script parameter.)</span></span>

<span data-ttu-id="3a8cf-359">결과 다음과 같이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-359">The result should look like the following:</span></span>

[!code-xml[Main](the-fix-it-sample-application/samples/sample32.xml)]

<span data-ttu-id="3a8cf-360">같은 MyFixIt.WorkerRoler\app.config 파일에 아래 `appSettings`, Azure 저장소 계정에 대 한 두 자리 표시자 값을 대체 합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-360">In the same MyFixIt.WorkerRoler\app.config file, under `appSettings`, replace the two placeholder values for the Azure storage account.</span></span>

[!code-xml[Main](the-fix-it-sample-application/samples/sample33.xml?highlight=2-3)]

<span data-ttu-id="3a8cf-361">포털에서 액세스 키를 가져올 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-361">You can get the access key from the portal.</span></span> <span data-ttu-id="3a8cf-362">참조 [저장소 계정을 관리 하는 방법을](https://docs.microsoft.com/azure/storage/common/storage-create-storage-account)합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-362">See [How To Manage Storage Accounts](https://docs.microsoft.com/azure/storage/common/storage-create-storage-account).</span></span>

<span data-ttu-id="3a8cf-363">MyFixItCloudService\ServiceConfiguration.Cloud.cscfg, Azure 저장소 계정에 대 한 동일한 두 자리 표시자 값을 대체 합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-363">In MyFixItCloudService\ServiceConfiguration.Cloud.cscfg, replace the same two placeholders values for the Azure storage account.</span></span>

[!code-xml[Main](the-fix-it-sample-application/samples/sample34.xml?highlight=3)]

<span data-ttu-id="3a8cf-364">이제 클라우드 서비스를 배포할 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-364">Now you are ready to deploy the cloud service.</span></span> <span data-ttu-id="3a8cf-365">솔루션 탐색기 MyFixItCloudService 프로젝트를 마우스 오른쪽 단추로 클릭 하 고 선택 **게시**합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-365">In Solution Explore, right-click the MyFixItCloudService project and select **Publish**.</span></span> <span data-ttu-id="3a8cf-366">자세한 내용은 참조 하십시오. "[Azure에 응용 프로그램을 배포](https://www.windowsazure.com/develop/net/tutorials/multi-tier-web-site/2-download-and-run/#deployAz)", 2 부에 [이 자습서](https://code.msdn.microsoft.com/Windows-Azure-Multi-Tier-eadceb36)합니다.</span><span class="sxs-lookup"><span data-stu-id="3a8cf-366">For more information, see "[Deploy the Application to Azure](https://www.windowsazure.com/develop/net/tutorials/multi-tier-web-site/2-download-and-run/#deployAz)", which is in part 2 of [this tutorial](https://code.msdn.microsoft.com/Windows-Azure-Multi-Tier-eadceb36).</span></span>

>[!div class="step-by-step"]
[<span data-ttu-id="3a8cf-367">이전</span><span class="sxs-lookup"><span data-stu-id="3a8cf-367">Previous</span></span>](more-patterns-and-guidance.md)
