...
보낸 바이트 수에는 브라우저 요청에 적용 되는 HTTP 헤더를 사용 하 여 매우 자세한 정보는 번들 상당히 감소를 했습니다. 수신된 바이트 수를 줄이는으로 크지 않습니다. 때문에 가장 큰 파일 (*스크립트\\jquery-ui-1.8.11.min.js* 하 고 *스크립트\\jquery 1.7.1.min.js*) 이미 축소 되기 . 참고: 사용 하는 샘플 프로그램에 타이밍 합니다 [Fiddler](http://www.fiddler2.com/fiddler2/) 느린 네트워크를 시뮬레이션 하는 도구입니다. (의 Fiddler에서 **규칙** 메뉴에서 **성능** 한 다음 **모뎀 속도 시뮬레이션**.)
...
4. 에 **검색 스크립트** 입력 상자, 디버깅 하려는 함수의 이름을 선택 합니다. 다음 그림과에서 **AddAltToImg** 에 입력 된 합니다 **검색 스크립트** 입력란입니다.  
...
엽니다는 *앱\\\_시작\\BundleConfig.cs* 파일을 검사 합니다 `RegisterBundles` 만들기, 등록 및 번들을 구성 하는 데 사용 되는 메서드. 다음 코드의 일부를 보여 줍니다.는 `RegisterBundles` 메서드.
...
- ".Min" 파일을 선택 하는 경우 릴리스 *FileX.min.js* 및 *FileX.js* 존재 합니다.
- 디버그에 대 한 비 ".min" 버전을 선택합니다.
- 무시 하 고 "-vsdoc" 파일 (같은 *jquery-1.7.1-vsdoc.js*), IntelliSense 에서만 사용 되는 합니다.
...
합니다 [번들](https://msdn.microsoft.com/library/system.web.optimization.bundle(v=VS.110).aspx) 클래스 `Include` 메서드는 여기서 각 문자열은 리소스의 가상 경로 문자열의 배열을 사용 합니다. 다음 코드를 `RegisterBundles` 의 메서드를 *App\\\_시작\\BundleConfig.cs* 파일은 번들 추가할 여러 파일을 보여 줍니다.:
...
번들은 렌더링 메서드를 사용 하는 뷰에서 참조 하는 (`Styles.Render` css 및 `Scripts.Render` JavaScript 용). 다음 태그를 *뷰\\공유\\\_Layout.cshtml* 파일은 기본 ASP.NET 인터넷 프로젝트 보기 CSS 및 JavaScript 번들을 참조 하는 방법을 보여 줍니다.
...
- *스크립트\\일반적인\\AddAltToImg.js*
- *스크립트\\일반적인\\ToggleDiv.js*
- *스크립트\\일반적인\\ToggleImg.js*
- *스크립트\\일반적인\\Sub1\\ToggleLinks.js*
...
| --- | --- |
| Include("~/Scripts/Common/\*.js") | *AddAltToImg.js*하십시오 *ToggleDiv.js*, *ToggleImg.js* |
...
| IncludeDirectory ("~/Scripts/Common", "T\*", true) | *ToggleDiv.js*하십시오 *ToggleImg.js*, *ToggleLinks.js* |
...
- 다른 파일을 가져올 CSS 파일 두 번 로드 하는 가져온된 파일에서 발생 합니다. 예를 들어, 다음 코드는 대부분의 jQuery UI 테마 CSS 파일이 두 번 로드를 사용 하 여 번들을 만듭니다. 
  ...
  와일드 카드 선택기 "\*.css" 폴더에 각 CSS 파일에는 포함 하 여 합니다 *콘텐츠\\테마\\기본\\jquery.ui.all.css* 파일. 합니다 *jquery.ui.all.css* 파일 다른 CSS 파일을 가져옵니다.
...
1. 작은 콘텐츠에 대 한 폴더를 만듭니다. 다음 예제에서는 합니다 *콘텐츠\\MyLess* 폴더입니다.
...
4. 사용 하 여 LESS 파일의 번들을 만들지 합니다 `LessTransform` 하며 [CssMinify](https://msdn.microsoft.com/library/system.web.optimization.cssminify(VS.110).aspx) 변환 합니다. 다음 코드를 추가 합니다 `RegisterBundles` 의 메서드를 *앱\\시작 (_s)\\BundleConfig.cs* 파일입니다.
...
합니다 `System.Web.Optimization` 네임 스페이스에서 구현 됩니다 *System.Web.Optimization.dll*합니다. WebGrease 라이브러리를 활용 하 여 (*WebGrease.dll*) 축소 기능을 다시 사용 하는 *Antlr3.Runtime.dll*합니다.
