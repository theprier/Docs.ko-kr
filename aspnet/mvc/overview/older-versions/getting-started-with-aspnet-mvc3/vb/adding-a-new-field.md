...
> 
> 이 항목에 수반 VB.NET 소스 코드를 사용 하 여 Visual Web Developer 프로젝트 ´ ù. [VB.NET 버전을 다운로드](https://code.msdn.microsoft.com/Introduction-to-MVC-3-10d1b098)합니다. 원하는 경우 C#으로 전환 된 [C# 버전](../cs/adding-a-new-field.md) 이 자습서의 합니다.
...
을 열고는 *\Views\Movies\Create.vbhtml* 파일을 폼의 끝 부분에 다음 태그를 추가 합니다. 이 렌더링 하는 텍스트 상자가 새 동영상 만들어질 때에 대 한 등급을 지정할 수 있습니다.
...
클래스의 이름을 &quot;MovieInitializer&quot;합니다. 업데이트는 `MovieInitializer` 다음 코드를 포함 하는 클래스:
...
찾을 `Application_Start` 메서드 호출을 추가 하 고 `Database.SetInitializer` 아래와 같이 메서드의 시작 부분에서:
