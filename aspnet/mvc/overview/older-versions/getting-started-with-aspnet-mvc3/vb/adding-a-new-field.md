...
업데이트 했으므로 합니다 `Model` 클래스도 업데이트 해야 합니다 *\Views\Movies\Index.vbhtml* 및 *\Views\Movies\Create.vbhtml* 새 를지원하기위해템플릿을보려면`Rating`속성입니다.

엽니다는<em>\Views\Movies\Index.vbhtml</em> 파일을 추가 `<th>Rating</th>` 열 머리글 바로 뒤를 <strong>가격</strong> 열입니다. 추가한를 `<td>` 렌더링 템플릿의 끝 열을 `@item.Rating` 값입니다. 업데이트 된 다음과 같습니다 <em>Index.vbhtml</em> 보기 템플릿은 다음과 같습니다.

[!code-vbhtml[Main](adding-a-new-field/samples/sample3.vbhtml)]

다음으로 열고 합니다 *\Views\Movies\Create.vbhtml* 파일과 폼의 끝 부분에 다음 태그를 추가 합니다. 이 입력란을 렌더링 하는 새 영화를 만들 때 등급을 지정할 수 있습니다.
...
클래스의 이름을 &quot;MovieInitializer&quot;합니다. 업데이트 된 `MovieInitializer` 다음 코드를 포함 하는 클래스:
