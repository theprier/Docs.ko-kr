...
**그림 01**: 캐시 시간 ([전체 크기 이미지를 보려면 클릭](understanding-action-filters-vb/_static/image3.png))
...
모든 작업 필터에 대 한 기본 클래스는는 `System.Web.Mvc.FilterAttribute` 클래스입니다. 특정 형식의 필터를 구현 하려면 다음 기본 필터 클래스에서 상속 하 고 IResultFilter, IActionFilter, IAuthorizationFilter 중 하나 이상을 구현 하는 클래스를 만들어야 할 또는 ExceptionFilter 인터페이스입니다.
...
기본 ActionFilterAttribute 클래스에 재정의할 수 있는 다음과 같은 메서드가 있습니다.
