...
---
<a name="improving-performance-with-output-caching-c"></a>출력 캐싱 (C#)를 사용 하 여 성능 향상
====================
...
개별 컨트롤러 작업 또는 전체 컨트롤러 클래스 중 하나를 [OutputCache] 특성을 추가 하 여 출력 캐싱을 사용 합니다. 예를 들어 목록 1에서 컨트롤러 index () 라는 작업을 노출 합니다. Index () 작업의 출력은 10 초 동안 캐시 됩니다.
...
기본적으로 [OutputCache] 특성을 사용 하는 경우 콘텐츠가 캐시 세 위치에서: 웹 서버, 모든 프록시 서버 및 웹 브라우저입니다. 콘텐츠 [OutputCache] 특성의 위치 속성을 수정 하 여 캐시 된 위치에 정확 하 게 제어할 수 있습니다.
...
**3 – Controllers\BadUserController.cs 나열**
...
**4-Controllers\UserController.cs 나열**

[!code-csharp[Main](improving-performance-with-output-caching-cs/samples/sample4.cs)]

목록 4 [OutputCache] 특성 OutputCacheLocation.Client 값으로 설정 된 위치 속성에 알 수 있습니다. [OutputCache] 특성 NoStore 속성을도 포함 되어 있습니다. NoStore 속성은 캐시 된 콘텐츠를의 영구 복사본을 저장 하지 마십시오 프록시 서버 및 브라우저 알리는 데 사용 됩니다.
...
VaryByParam 속성 [OutputCache] 특성을 활용 하 여이 문제를 해결할 수 있습니다. 이 속성을 사용 하면 서로 다른 캐시 된 버전 때 폼 매개 변수와 동일한 콘텐츠를 만들 수 있습니다 또는 쿼리 문자열 매개 변수 달라 집니다.

예를 들어 목록 5의 컨트롤러 Master() 및 Details() 라는 두 가지 작업을 노출 합니다. Master() 작업 영화 타이틀의 목록을 반환 하 고 Details() 작업 선택한 동영상에 대 한 세부 정보를 반환 합니다.

**5-Controllers\MoviesController.cs 나열**
...
[OutputCache] 특성의 속성을 수정 하 여 출력 캐시 속성을 구성 하는 대신, 웹 구성 (web.config) 파일에서 캐시 프로필을 만들 수 있습니다. 웹 구성 파일에서 캐시 프로필을 만드는 몇 가지 중요 한 이점 제공 합니다.
...
7의 컨트롤러 Cache1Hour 프로필 [OutputCache] 특성으로 컨트롤러 작업에 적용 하는 방법을 보여 줍니다.
...
출력 캐싱을 크게 ASP.NET MVC 응용 프로그램의 성능을 향상 시키기 위해 쉽게 메서드를 사용 하 여 제공 합니다. 이 자습서에서는 컨트롤러 작업의 출력을 캐시 하기 위해 [OutputCache] 특성을 사용 하는 방법을 알아보았습니다. 콘텐츠 캐시 되는 방식을 수정 하려면 기간과 VaryByParam 속성과 같은 [OutputCache] 특성의 속성을 수정 하는 방법을 알아보았습니다. 마지막으로 웹 구성 파일에서 캐시 프로필을 정의 하는 방법을 알아보았습니다.
