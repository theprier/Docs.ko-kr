...
- [ASP.NET 데이터 액세스 옵션](https://msdn.microsoft.com/en-us/library/ms178359.aspx) (MSDN). ASP.NET에 대 한 관계형 데이터베이스에 대 한 데이터 액세스 옵션와 플랫폼을 선택 하 고 사용 중인 시나리오에 적합 한 메서드에 액세스 하는 방법에 대 한 지침을 소개 합니다.
...
- [Entity Framework 개발 방법](https://msdn.microsoft.com/en-us/library/ms178359.aspx#dbfmfcf) (MSDN). Entity Framework 개발 방법 Database First Model First 또는 Code First를 선택 하는 방법에 대 한 지침입니다.
...
- [Entity Framework-기존 데이터베이스에 먼저 코드](https://msdn.microsoft.com/en-us/data/jj200620)합니다. MSDN 합니다. 비디오 및 기존 데이터베이스와 함께 Code First를 사용 하는 방법을 보여 주는 연습입니다.
- [데이터 개발자 센터-Entity Framework](https://msdn.microsoft.com/data/ef)합니다. MSDN 합니다. 생성 되 고 Entity Framework 팀에서 유지 관리 하는 Entity Framework 설명서에 대 한 지침을 참조 하십시오.는 [시작](https://msdn.microsoft.com/en-us/data/ee712907) 링크 합니다.
...
- [Windows Azure 웹 사이트에 멤버 자격, OAuth, SQL 데이터베이스와 보안 ASP.NET MVC 5 앱을 배포](https://docs.microsoft.com/aspnet/core/security/authorization/secure-data)합니다. Microsoft Azure)입니다. 멤버 자격 및 응용 프로그램 데이터를 Azure에 배포할 마이그레이션을 사용 하는 방법.
- [Visual Studio 및 ASP.NET에 대 한 웹 배포 개요](https://msdn.microsoft.com/en-us/library/dd394698.aspx#dbdeployment)합니다. 참조는 **Visual Studio에서 데이터베이스 배포 구성** Code First 마이그레이션을 Visual Studio 웹 배포 기능에 통합 하는 방법에 대 한 설명은 섹션.
- [데이터 개발자 센터 Code First 마이그레이션](https://msdn.microsoft.com/en-us/data/jj591621) (MSDN). Entity Framework 팀의 마이그레이션 설명서입니다.
...
- [데이터 개발자 센터-관련된 엔터티 로드](https://msdn.microsoft.com/en-us/data/jj574232) (MSDN). 로드에 대 한 설명서는 Entity Framework 팀의 관련 데이터를 사용 합니다.
...
- [Entity Framework 5에 대 한 성능 고려 사항](https://msdn.microsoft.com/en-us/data/hh949853) (MSDN).
- [성능 고려 사항 (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc853327) (MSDN).
...
- [데이터 개발자 센터 – 낙관적 동시성 패턴](https://msdn.microsoft.com/en-us/data/jj592904) (MSDN). Entity Framework 팀의 동시성 설명서입니다.
...
이러한도 모두 최신 버전 현재 몇 가지 권장된 기법으로 유지 합니다. 인터넷에서 사용할 수 있는 것 보다는 Entity Framework에 대 한 보다 포괄적인 아직 따라 하기 쉬운 소개를 제공합니다. 다른 책 [Entity Framework 프로그래밍](http://shop.oreilly.com/product/9780596807252.do) 으로 Julie Lerman 크고 복잡 하지만 오래 된 이며 적용 되는 기술 중 상당수는 더 이상 없으며 Entity Framework를 사용 하는 것이 좋습니다. 참고 항목에서 Entity Framework 팀에서 권장 하는 책 목록을 [데이터 개발자 센터 설명서](https://msdn.microsoft.com/en-us/data/aa937716) MSDN 사이트입니다.

<a id="otherefresources"></a>

### <a name="other-entity-framework-resources"></a>다른 Entity Framework 리소스

- [Entity Framework (ADO.NET) 팀 블로그](https://blogs.msdn.com/b/adonet/)합니다. 최신 정보와 공지 향상 된 새로운 기능에 대 한 최상의 리소스 중 하나입니다. 다른 EF 관련 블로그 참조에서 Blogroll [Entity Framework와 함께 시작](https://msdn.microsoft.com/en-us/data/ee712907)합니다.
- [MSDN Magazine](https://msdn.microsoft.com/en-us/magazine/default.aspx)합니다. 참조는 **데이터 요소** 열은 Entity Framework와 관련 된 항목에 대 한 경우가 많습니다.

<a id="wfdatabinding"></a>

## <a name="data-binding-in-aspnet-web-forms-applications"></a>ASP.NET 웹에서 데이터 바인딩 Forms 응용 프로그램

- [ASP.NET Web Forms 데이터 액세스 옵션](https://msdn.microsoft.com/en-us/library/jj822927.aspx) (MSDN)<a id="wfmodelbinding"></a>합니다.
...
- [데이터 소스 웹 서버 컨트롤](https://msdn.microsoft.com/en-us/library/ms247258.aspx) (MSDN).
...
- [데이터 바인딩된 컨트롤을 웹 서버 컨트롤](https://msdn.microsoft.com/en-us/library/ms228214.aspx) (MSDN).
- [데이터 바인딩 식 개요](https://msdn.microsoft.com/en-us/library/ms178366.aspx) (MSDN). 이 페이지를 포함 **Eval** 및 **바인딩할**; 포함 하도록 업데이트 되지 않은 **항목** 및 **BindItem**합니다.

<a id="sqlserver"></a>

## <a name="working-with-sql-server-databases"></a>SQL Server 데이터베이스 작업

- [SQL Server 데이터베이스 기능](https://msdn.microsoft.com/en-us/library/hh230827.aspx) (MSDN). 다양 한 SQL Server 항목에 대 한 일반 소개, 목차에서 아래 항목을 참조 하세요.
- [SQL Server 버전](https://msdn.microsoft.com/en-us/library/ms178359.aspx#sqlserver) (MSDN). 요약에 각각에 대 한 자세한 정보 링크를 사용할 수 있는 SQL Server 버전의.)
- [ASP.NET 웹 응용 프로그램에 대 한 SQL Server 연결 문자열](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [SQL Server Compact를 사용 하 여 ASP.NET 웹 응용 프로그램에 대 한](https://msdn.microsoft.com/en-us/library/ms247257.aspx) (MSDN).
...
- [SQL Server Express 2012 LocalDB](https://msdn.microsoft.com/en-us/library/hh510202(v=sql.110).aspx) (MSDN). LocalDB에 공식 MSDN 소개 합니다.
- [ASP.NET 웹 응용 프로그램에 대 한 SQL Server 연결 문자열](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN).
- [방법: LocalDB로 업그레이드](https://msdn.microsoft.com/en-us/library/hh873188.aspx) (MSDN). LocalDB에 이전 버전의 SQL Server Express.mdf 파일을 마이그레이션하는 방법입니다. 중 하나를 다운로드 하는 경우에이 프로세스를 진행할 수 있습니다는 [SQL Server 2012 예제 데이터베이스](https://go.microsoft.com/fwlink/?linkid=117483)합니다.
...
- [ASP.NET 웹 응용 프로그램에 대 한 SQL Server 연결 문자열](https://msdn.microsoft.com/en-us/library/jj653752.aspx) (MSDN). AttachDBFileName 연결 문자열 설정을 SQL Server express를 사용 하면 특히이 페이지의 사용자 인스턴스 섹션을 참조 합니다.
...
- [내 ASP.NET 웹 응용 프로그램 프로덕션 환경에서 SQL Server Express 데이터베이스를 사용할 수 있습니까?](https://msdn.microsoft.com/en-us/library/jj653753.aspx#sql_express_in_production) (MSDN)입니다.
...
- [Windows Azure SQL 데이터베이스](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279.aspx) (MSDN). MSDN의 SQL 데이터베이스에 대 한 목차의 최상위 노드.
...
- [일시적인 오류 처리 응용 프로그램 블록](https://msdn.microsoft.com/en-us/library/hh680934(v=PandP.50).aspx)합니다. 수 있는 일시적인 네트워크 오류와 연결 오류 처리 제한에서 해당 결과 프레임 워크입니다. NuGet 패키지에서 사용할 수 있는: [Enterprise Library 5.0-일시적인 오류 처리 응용 프로그램 블록](http://nuget.org/packages/EnterpriseLibrary.WindowsAzure.TransientFaultHandling)합니다.
- [SQL 데이터베이스 및 Entity Framework 시작](https://msdn.microsoft.com/en-us/data/jj556244) (MSDN).
- [Windows Azure 트레이닝 키트](https://www.microsoft.com/en-us/download/details.aspx?id=8396) (Microsoft 다운로드 센터). SQL 데이터베이스에 대 한 실습 랩을 포함합니다.
- [Windows Azure SQL 데이터베이스 커뮤니티 포럼](https://social.msdn.microsoft.com/Forums/en-US/ssdsgetstarted/threads)합니다.
- [Windows Azure SQL 데이터베이스를 이동](https://msdn.microsoft.com/en-us/library/ff803375.aspx) (MSDN). Microsoft Patterns and Practices 팀에서 포괄적인 종단 간 시나리오의 한 장 합니다. 마이그레이션하는 이유 표지와 SQL 데이터베이스에 SQL Server에서 마이그레이션하는 방법입니다.
...
- [Windows Azure SQL 데이터베이스로 데이터 마이그레이션: 도구와 기술을](https://msdn.microsoft.com/en-us/library/windowsazure/hh694043.aspx) (MSDN). SQL 데이터베이스에 SQL Server를 비교 하 고 SQL 데이터베이스에 SQL Server에서 마이그레이션하는 경우에 대 한 지침을 제공 하는 섹션이 포함 됩니다.
...
- [SQL Server 기능 제한 (Windows Azure SQL 데이터베이스)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394115.aspx) (MSDN).
- [Windows Azure 테이블 저장소 및 Windows Azure SQL 데이터베이스-비교 및 대조](https://msdn.microsoft.com/en-us/library/jj553018.aspx) (MSDN). Windows Azure에 배포 하는 응용 프로그램에 대 한 Windows Azure 테이블 저장소는 Windows Azure SQL 데이터베이스에 대 한 대안을 수 있습니다. 이 항목에는 이러한 대체 방법 중에서 결정할 수 있습니다.
- [Windows Azure SQL 데이터베이스](https://msdn.microsoft.com/en-us/library/windowsazure/ee336279) (MSDN).
- [지침 및 제한 사항 (Windows Azure SQL 데이터베이스)](https://msdn.microsoft.com/en-us/library/windowsazure/ff394102.aspx)

<a id="nosql"></a>

## <a name="working-with-nosql-database-management-systems"></a>NoSQL 데이터베이스 관리 시스템으로 작업

- [Windows Azure 데이터 서비스](https://www.windowsazure.com/en-us/develop/net/data/) (Microsoft Azure 사이트). 참조는 [테이블 서비스 기능 가이드](https://docs.microsoft.com/azure/) 및 **빅 데이터** 페이지의 섹션입니다.
...
- [ASP.NET 데이터 액세스 옵션](https://msdn.microsoft.com/en-us/library/ms178359.aspx#linq) (MSDN). LINQ 소개에 포함 되어 있습니다.
...
- [동적 데이터 프로젝트 템플릿](https://msdn.microsoft.com/en-us/library/jj822927.aspx#dynamicdata) (MSDN). 동적 데이터 프로젝트를 사용 하는 경우에 대 한 지침입니다.
...
- [ASP.NET 데이터 액세스 보안](https://msdn.microsoft.com/en-us/library/ms178375.aspx) (MSDN).
- [보안 고려 사항 (Entity Framework)](https://msdn.microsoft.com/en-us/library/cc716760.aspx) (MSDN).
- [방법: 데이터 소스 제어를 사용 하는 경우 연결 문자열을 보호](https://msdn.microsoft.com/en-us/library/ms178372.aspx) (MSDN).

<a id="optimizingdataaccess"></a>

## <a name="optimizing-data-access-performance"></a>데이터 액세스 성능 최적화

- [ASP.NET 성능 개요](https://msdn.microsoft.com/en-us/library/cc668225.aspx) (MSDN).
- [ASP.NET 캐싱](https://msdn.microsoft.com/en-us/library/xsbfdd8c.aspx) (MSDN).
- [ASP.NET 성능 향상](https://msdn.microsoft.com/en-us/library/ff647787) (MSDN). 이 페이지 맨 위에 있는 "콘텐츠 사용 되지 않음" 경고 하지만 대부분의 정보는도 적용 되며 업데이트 된 리소스가 없으면 비교할 수 있습니다.
- [SQL Server 성능 향상](https://msdn.microsoft.com/en-us/library/ff647793) (MSDN). 이전 링크와 같은 설명 합니다.
...
- [웹 서비스를 통해 데이터 액세스](https://msdn.microsoft.com/en-us/library/ms178359.aspx#webservice) (MSDN). WCF 또는 웹 API를 사용 하는 경우에 대 한 지침입니다.
...
- [ASP.NET 데이터 액세스 FAQ](https://msdn.microsoft.com/en-us/library/jj653753.aspx) (MSDN).
- [ASP.NET Web Forms 자습서-데이터](../web-forms/overview/data-access/index.md)합니다. 이 자습서의 대부분은 상대적으로 이전; 읽어보십시오 [ASP.NET 데이터 액세스 옵션](https://msdn.microsoft.com/en-us/library/ms178359.aspx) 및 [데이터 저장소 옵션 (실제 클라우드로 응용 프로그램 빌딩 Windows Azure)](../aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/data-storage-options.md) 첫 번째 오른쪽 없는 데이터 액세스 방법에 너무 큰를 얻지 있도록 시나리오입니다.
...
- [Visual Studio에서 데이터 액세스](https://msdn.microsoft.com/en-us/library/wzabh8c4.aspx) (MSDN). 새 링크 목록을 제공이 콘텐츠 맵은 있지만 ASP.NET 보다는 Visual Studio에 중점을 두고 있습니다.
