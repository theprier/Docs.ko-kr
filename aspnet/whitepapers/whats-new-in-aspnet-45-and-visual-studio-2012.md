...
.NET Framework 4.5 및 Windows 8 웹 서버 워크 로드에 대 한 상당한 성능 향상을 달성 하는 데 도움이 되는 기능을 소개 합니다. 여기에 감소 (최대 35%) 모두 시작 시간 및 ASP.NET을 사용 하는 사이트를 호스트 하는 웹의 메모리 공간입니다.
...
이 문제를 해결 하기 위해 ASP.NET 4.5 컨트롤에 바인딩되는 데이터의 데이터 형식을 선언 하는 기능을 추가 합니다. 이렇게 하면 새 *ItemType* 속성입니다. 데이터 바인딩 식의 범위에 두 개의 새 형식화 된 변수를 사용할 수 있습니다이 속성을 설정 하는 경우: *항목* 하 고 *binditem을*입니다. 변수는 강력한 형식 이므로 Visual Studio 개발 환경의 모든 혜택을 얻을 수 있습니다.
...
이제 HTML 인코딩 데이터 바인딩 식의 결과를 것 있습니다. 콜론 (:)의 끝에 추가 된 &lt;% # 데이터 바인딩 식을 표시 하는 접두사:
