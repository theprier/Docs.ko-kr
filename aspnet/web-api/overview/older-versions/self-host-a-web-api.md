...
NuGet 패키지 관리자가 설치 되어 있는지를 확인 하려면 클릭 합니다 **도구** Visual Studio의 메뉴. 항목 이라는 메뉴가 나타나면 **NuGet 패키지 관리자**, NuGet 패키지 관리자를 해야 합니다.
...
1. **도구** 메뉴에서 **NuGet 패키지 관리자**합니다. *참고*: 경우 하면이 메뉴가 표시 되지 않으면이 항목을 해당 NuGet 패키지 관리자를 제대로 설치 되어 있는지 확인 합니다.
