...
이미지 태그 도우미는 `img` (`<img>`) 태그의 기능을 향상시킵니다. 이미지 태그 도우미는 `boolean` 특성인 `asp-append-version`을 비롯해서 `src` 특성을 필요로 합니다.
...
정적 파일이 *..wwwroot/images/asplogo.png* 디렉터리에 위치해 있을 경우, 생성되는 HTML은 다음과 비슷합니다 (해시는 다를 것입니다).
