...
ASP.NET Core는 [종속성 주입](../../fundamentals/dependency-injection.md)을 기본적으로 지원하므로 애플리케이션을 더 쉽게 테스트하고 유지 관리할 수 있습니다.

## <a name="constructor-injection"></a>생성자 주입

생성자 기반 종속성 주입을 위한 ASP.NET Core의 기본 지원은 MVC 컨트롤러까지 확장합니다. 컨트롤러에 서비스 형식을 생성자 매개 변수로 추가하기만 하면 ASP.NET Core는 서비스 컨테이너의 기본 기능을 사용하여 해당 형식을 확인하려고 시도합니다. 서비스는 항상 그렇지는 않지만 일반적으로 인터페이스를 사용하여 정의됩니다. 예를 들어 현재 시간에 따라 달라지는 비즈니스 논리가 애플리케이션에 있는 경우, 설정된 시간을 사용하는 구현에서 테스트가 통과될 수 있도록 시간을 검색하는(하드코딩이 아님) 서비스를 주입할 수 있습니다.
...
지금 애플리케이션을 실행하는 경우 오류가 발생할 가능성이 있습니다.
...
서비스를 한 번 구성하면 애플리케이션을 실행하고 홈 페이지로 이동할 때 시간 기반 메시지가 예상대로 표시되어야 합니다.

![서버 인사말](dependency-injection/_static/server-greeting.png)

>[!TIP]
> 컨트롤러에서 종속성을 명시적으로 요청하여 코드를 더 쉽게 테스트할 수 있는 방법을 알아보려면 [컨트롤러 논리 테스트](testing.md)를 참조하세요.
...
컨트롤러 내에서 애플리케이션 또는 구성 설정에 액세스하는 것은 일반적인 패턴입니다. 이 액세스는 [구성](xref:fundamentals/configuration/index)에 설명된 옵션 패턴을 사용해야 합니다. 일반적으로 종속성 주입을 사용하여 컨트롤러에서 직접 설정을 요청해서는 안 됩니다. 더 나은 방법은 `IOptions<T>` 인스턴스를 요청하는 것입니다. 여기서 `T`란 필요한 구성 클래스입니다.

옵션 패턴을 사용하려면 다음과 같은 옵션을 나타내는 클래스를 만들어야 합니다.

[!code-csharp[](dependency-injection/sample/src/ControllerDI/Model/SampleWebSettings.cs)]

그런 다음, 옵션 모델을 사용하도록 애플리케이션을 구성하고, 구성 클래스를 `ConfigureServices`의 서비스 컬렉션에 추가해야 합니다.

[!code-csharp[](./dependency-injection/sample/src/ControllerDI/Startup.cs?highlight=3,4,5,6,9,16,19&range=14-44)]

> [!NOTE]
> 위의 목록에서 JSON 형식 파일에서 설정을 읽도록 애플리케이션을 구성하는 중 입니다. 또한 위의 주석으로 처리된 코드에 표시된 대로 코드에서 완전히 설정을 구성할 수 있습니다. 추가 구성 옵션은 [구성](xref:fundamentals/configuration/index)을 참조하세요.
...
옵션 패턴을 따르면 설정 및 구성을 서로 분리할 수 있으며, 컨트롤러가 [문제의 분리](/dotnet/standard/modern-web-apps-azure-architecture/architectural-principles#separation-of-concerns)를 따르고 있는지 확인합니다. 컨트롤러는 설정 정보를 찾는 방법이나 위치를 알 필요가 없기 때문입니다. 또한 컨트롤러가 더 쉽게 [단위 테스트](testing.md)를 수행할 수 있습니다. 컨트롤러 클래스 내 설정 클래스의 직접 인스턴스화가 없기 때문입니다.
