...
> [!NOTE]
> Kerberos 인증 프로토콜을 사용 하 여 커널 모드 인증을 HTTP.sys 대리자입니다. Kerberos 및 HTTP.sys를 사용 하 여 사용자 모드 인증이 지원 되지 않습니다. 컴퓨터 계정이 Active Directory에서 가져온 Kerberos 토큰/티켓의 암호를 해독 하는 데 사용 하 고 사용자를 인증 하는 서버에 클라이언트에서 전달 해야 합니다. 앱의 사용자가 아닌 호스트에 대 한 서비스 주체 이름 (SPN)을 등록 합니다.
...
> [!NOTE]
> Kerberos 인증 프로토콜을 사용 하 여 커널 모드 인증에 WebListener 대리자입니다. Kerberos 및 WebListener를 사용 하 여 사용자 모드 인증이 지원 되지 않습니다. 컴퓨터 계정이 Active Directory에서 가져온 Kerberos 토큰/티켓의 암호를 해독 하는 데 사용 하 고 사용자를 인증 하는 서버에 클라이언트에서 전달 해야 합니다. 앱의 사용자가 아닌 호스트에 대 한 서비스 주체 이름 (SPN)을 등록 합니다.
...
> [!NOTE]
> 기본적으로 권한 부여를 페이지에 액세스할 수 없는 사용자는 빈 HTTP 403 응답으로 표시 됩니다. 합니다 [StatusCodePages 미들웨어](xref:fundamentals/error-handling#configuring-status-code-pages) "액세스 거부" 더 나은 환경을 제공 하기 위해 구성할 수 있습니다.
