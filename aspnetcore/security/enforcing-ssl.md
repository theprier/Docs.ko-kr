...
ASP.NET Core 2.1와 (Visual Studio 또는 dotnet 명령줄)에서 최신 웹 응용 프로그램 템플릿을 사용 하면 [HTTPS 리디렉션](#require) 및 [HSTS](#hsts)합니다. HTTPS를 필요로 하지 않는 배포의 경우 있습니다 수 옵트아웃 https입니다. 예를 들어 일부 백 엔드 서비스에 HTTPS 처리 되 고 외부에서 경계 면 HTTPS를 사용 하 여 각 노드에서 필요 하지 않습니다.
