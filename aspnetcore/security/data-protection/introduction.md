...
전체 문제 설명 단일 문장에 간결 하 게 정의할 수 있습니다: 나중에 검색에 대 한 신뢰할 수 있는 정보를 유지 해야 하지만 지 속성 메커니즘이 신뢰 하지 않습니다. 웹 측면에서이 수로 작성할 수 "싶습니다 신뢰할 수 없는 클라이언트를 통해 왕복 신뢰할 수 있는 상태로."
...
가능해 집니다 추가 제약 조건 요구 사항 범위를 좁히려면 합니다. 암호화 시스템 내에서 작동 하는 모든 서비스는 동일 하 게 신뢰할 수 있도록 하 고 데이터를 생성 하거나 우리의 직접 제어를 받는 서비스 외부에서 사용 된 필요 하지 않습니다 가정 합니다. 또한 작업은 가능한 한 빠르게 각 요청 웹 서비스에 한 번 이상 암호화는 시스템을 통해 이동할 수 있으므로 필요 합니다. 이로써 대칭 암호화 시나리오에 대 한 이상적인 며 등 필요한에 한 번까지 비대칭 암호화 할인 수 있습니다.
