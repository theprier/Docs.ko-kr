...
호출 [AddCors](/dotnet/api/microsoft.extensions.dependencyinjection.corsservicecollectionextensions.addcors) 에서 `Startup.ConfigureServices`:

[!code-csharp[](cors/sample/CorsExample1/Startup.cs?name=snippet_addcors)]

## <a name="enabling-cors-with-middleware"></a>미들웨어를 이용해서 CORS 활성화시키기

CORS를 사용 하도록 하 여 요청 파이프라인에 CORS 미들웨어를 추가 합니다 `UseCors` 확장 메서드. CORS 미들웨어 앞에 야 정의 된 끝점이 모든 앱에서 크로스-원본 요청을 지원 하려면 (예를 들어 호출 하기 전에 `UseMvc`).

크로스-원본 정책을 사용 하 여 CORS 미들웨어를 추가할 때 지정할 수는 [CorsPolicyBuilder](/dotnet/api/microsoft.extensions.dependencyinjection.corsservicecollectionextensions.addcors) 클래스입니다. 구체적인 방법은 두 가지입니다. 첫 번째 호출 방법은 `UseCors` 람다를 사용 하 여:
...
다음과 같이 CorsPolicyBuilder fluent API에 있으므로 메서드 호출을 연결할 수 있습니다.
...
일부 옵션에 대 한 읽기 하면 도움이 될 [CORS 어떻게 작동](#how-cors-works) 첫 번째입니다.
...
크로스-원본 자격 증명을 허용 하는 경우 주의 해야 합니다. 다른 도메인에서 웹 사이트 사용자의 지식 없이 사용자를 대신 하 여 앱에 로그인 한 사용자의 자격 증명을 보낼 수 있습니다. CORS 사양도 해당 설정에 따라 원본이 `"*"` (모든 원본) 올바르지 않습니다 경우는 `Access-Control-Allow-Credentials` 헤더가 있습니다.
...
크로스-원본 요청 예제는 다음과 같습니다. `Origin` 헤더는 요청 하는 사이트의 도메인을 제공 합니다.
...
응답에는 액세스 제어-허용-원본 헤더가 포함 되지 않은, AJAX 요청이 실패 합니다. 특히 브라우저 요청을 허용 하지 않습니다. 서버 응답에 성공 하면 반환 하는 경우에 브라우저 되어도 응답 클라이언트 응용 프로그램에 사용할 수 있습니다.

### <a name="preflight-requests"></a>실행 전 요청

브라우저는 일부 CORS 요청에 대 한 리소스에 대 한 실제 요청을 보내기 전에 "실행 전 요청을" 라는 추가 요청을 보냅니다. 다음 조건이 true 인 경우 브라우저가 실행 전 요청을 건너뛸 수 있습니다.:

* 요청 메서드는 GET, HEAD 또는 POST 및

* 응용 프로그램 동의 수용-언어 Content-language 이외의 모든 요청 헤더를 설정 하지 않는 콘텐츠 형식 또는 마지막-이벤트-ID 및
...
요청 헤더에 대 한 규칙 setRequestHeader XMLHttpRequest 개체에서 호출 하 여 응용 프로그램을 설정 하는 헤더에 적용 됩니다. (CORS 사양 이러한 "작성자 요청 헤더"를 호출 합니다.) 규칙은 브라우저 등을 설정할 수, 사용자 에이전트, 호스트 또는 Content-length 헤더에 적용 되지 않습니다.
...
* 컨트롤-요청-헤더에 액세스: 응용 프로그램에 실제로 요청에 설정 된 요청 헤더의 목록입니다. (다시이 포함 되지 않습니다 브라우저 설정 하는 헤더.)
