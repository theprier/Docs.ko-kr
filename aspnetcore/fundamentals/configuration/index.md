...
[예제 코드 살펴보기 및 다운로드](https://github.com/aspnet/Docs/tree/master/aspnetcore/fundamentals/configuration/index/samples)([다운로드 방법](xref:tutorials/index#how-to-download-a-sample))
...
`AddEnvironmentVariables`는 <xref:Microsoft.AspNetCore.WebHost.CreateDefaultBuilder*>를 사용하여 새 <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder>를 초기화할 때 자동으로 호출됩니다. 자세한 내용은 [웹 호스트: 호스트 설정](xref:fundamentals/host/web-host#set-up-a-host)을 참조하세요.
...
| `ArrayExamples.Entries` 인덱스 | `ArrayExamples.Entries` 값 |
| :---------------------------: | :---------------------------: |
| 0                             | value0                        |
| 1                             | value1                        |
| 2                             | value2                        |
...
인덱스 &num;3에 대한 누락된 구성 항목은 `ArrayExamples` 인스턴스에 바인딩하기 전에 구성에서 올바른 키-값 쌍을 생성하는 모든 구성 공급자에서 제공할 수 있습니다. 샘플에 누락된 키-값 쌍이 있는 추가 JSON 구성 공급자가 포함된 경우 `ArrayExamples.Entries`는 전체 구성 배열과 일치합니다.
...
JSON 구성 공급자에 인덱스 &num;3에 대한 항목이 포함된 후 `ArrayExamples` 클래스 인스턴스를 바인딩하는 경우 `ArrayExamples.Entries` 배열에 이 값이 포함됩니다.
