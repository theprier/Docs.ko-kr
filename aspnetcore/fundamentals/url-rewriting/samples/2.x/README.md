...
이 샘플에는 URL(**https://localhost:5001**, **https://localhost**)을 사용하는 앱에 대한 `WebHostBuilder` 구성 및 이러한 리디렉션 메서드를 살펴보는 데 도움이 되는 테스트 인증서(**testCert.pfx**)가 포함되어 있습니다. **Startup.cs**의 `RewriteOptions()`에 이 중 하나를 추가하여 동작을 살펴봅니다.
