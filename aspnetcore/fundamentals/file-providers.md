...
[예제 코드 보기 및 다운로드](https://github.com/aspnet/Docs/tree/master/aspnetcore/fundamentals/file-providers/sample)
...
컨트롤러에서 공급자를 요청하려면 공급자를 컨트롤러의 생성자에서 지정한 다음, 로컬 필드에 할당합니다. 그리고 액션 메서드에서는 이 로컬 인스턴스를 사용합니다.
...
앞에서 구성한 물리적 공급자와 임베디드 공급자를 모두 포함하는 `CompositeFileProvider`를 사용하도록 예제 응용 프로그램을 업데이트하면 다음과 같은 결과가 출력됩니다.
...
![dotnet을 실행한 후, 명령 창에서 quotes.txt 파일을 모니터링해서 변경 내용을 확인하고 파일이 다섯 번 변경되었음을 보여줍니다.](file-providers/_static/watch-console.png)
...
**`directory/*/bower.json`**
   ...
   `directory` 디렉터리의 한 수준 아래에 있는 하위 디렉터리들의 모든 `bower.json` 파일을 찾습니다.

**<code>directory/&#42;&#42;/&#42;.txt</code>**

   `directory` 디렉터리 하위의 모든 위치에 존재하는 확장자가 `.txt`인 모든 파일을 찾습니다.
...
## <a name="recommendations-for-use-in-apps"></a>응용 프로그램에서 사용 시 권장 사항

ASP.NET Core 응용 프로그램에서 파일 시스템에 접근해야 할 때, 본문 예제에서 살펴본 것처럼 [종속성 주입](dependency-injection.md)을 통해서 `IFileProvider`의 인스턴스를 요청한 다음, 해당 인스턴스의 메서드를 사용해서 접근 작업을 수행할 수 있습니다. 이 방식을 사용하면 응용 프로그램이 시작될 때 한 번만 공급자를 구성해서 응용 프로그램이 생성하는 구현 형식의 인스턴스 개수를 줄일 수 있습니다.
