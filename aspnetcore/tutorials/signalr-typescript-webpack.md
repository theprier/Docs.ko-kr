...
[WebPack](https://webpack.js.org/)을 사용하여 개발자가 웹앱의 클라이언트 쪽 리소스를 번들링 및 빌드할 수 있습니다. 이 자습서에서는 해당 클라이언트가 [TypeScript](https://www.typescriptlang.org/)로 작성된 ASP.NET Core SignalR 웹앱에서 WebPack 사용을 보여 줍니다.
...
[예제 코드 살펴보기 및 다운로드](https://github.com/aspnet/Docs/tree/master/aspnetcore/tutorials/signalr-typescript-webpack/sample)([다운로드 방법](xref:tutorials/index#how-to-download-a-sample))
...
1. **도구** > **옵션** > **프로젝트 및 솔루션** > **웹 패키지 관리** > **외부 웹 도구**를 탐색합니다.
1. 목록에서 *$(PATH)* 항목을 선택합니다. 위쪽 화살표를 클릭하여 항목을 목록의 두 번째 위치로 이동합니다. 참고로 첫 번째 항목은 프로젝트의 로컬 패키지를 가리킵니다.
...
1. 프로젝트를 *SignalRWebPack*으로 이름 지정하고 **확인** 단추를 클릭합니다.
...
1. 다음 내용을 포함한 *src/css/main.css*를 생성합니다.
    ...
    앞의 *main.css* 파일은 앱의 스타일을 설정합니다.

1. 다음 내용을 포함한 *src/tsconfig.json*을 생성합니다.

    [!code-json[tsconfig.json](signalr-typescript-webpack/sample/src/tsconfig.json)]

    앞의 코드는 [ECMAScript](https://wikipedia.org/wiki/ECMAScript) 5 호환 JavaScript를 생성하도록 TypeScript 컴파일러를 구성합니다.
...
1. `Startup.Configure` 메서드에 제공된 코드는 *Hello World!* 를 표시합니다. `app.Run` 메서드를 [UseDefaultFiles](/dotnet/api/microsoft.aspnetcore.builder.defaultfilesextensions.usedefaultfiles#Microsoft_AspNetCore_Builder_DefaultFilesExtensions_UseDefaultFiles_Microsoft_AspNetCore_Builder_IApplicationBuilder_) 및 [UseStaticFiles](/dotnet/api/microsoft.aspnetcore.builder.staticfileextensions.usestaticfiles#Microsoft_AspNetCore_Builder_StaticFileExtensions_UseStaticFiles_Microsoft_AspNetCore_Builder_IApplicationBuilder_)에 대한 호출로 바꿉니다.
    ...
    앞의 코드는 사용자가 전체 URL을 입력하는지 아니면 웹앱의 루트 URL을 입력하는지에 따라 서버가 *index.html* 파일을 찾아서 제공할 수 있도록 합니다.
...
앱은 현재 메시지를 보낼 간단한 양식을 표시합니다. 그런 시도를 할 때 아무 일도 일어나지 않습니다. 서버는 특정 경로를 수신 대기하지만 보낸 메시지를 사용하여 아무 일도 하지 않습니다.
...
1. 이제 클라이언트가 메시지를 수신할 수 있으므로, 메시지를 보내도록 구성합니다. 강조 표시된 코드를 *src/index.ts* 파일에 추가합니다.
    ...
    WebSockets 연결을 통해 메시지를 보내면 `send` 메서드를 호출해야 합니다. 메서드의 첫 번째 매개 변수는 메시지 이름입니다. 메시지 데이터는 다른 매개 변수를 통해 지정합니다. 이 예에서 `newMessage`로 식별한 메시지를 서버에 보냅니다. 메시지는 사용자 이름 및 텍스트 상자에서의 사용자 입력으로 구성됩니다. 보내기가 작동하면 텍스트 상자 값이 지워집니다.
...
1. WebPack을 *릴리스* 모드로 실행합니다. **패키지 관리자 콘솔** 창을 사용하여 프로젝트 루트에서 다음 명령을 실행합니다.

    [!INCLUDE [npm-run-release](../includes/signalr-typescript-webpack/npm-run-release.md)]

1. **디버그** > **디버깅 없이 시작**을 선택하여 디버거를 연결하지 않고 브라우저에서 엽을 시작합니다. *wwwroot/index.html* 파일은 `http://localhost:<port_number>`에 제공됩니다.
...
1. 프로젝트 루트에서 다음 명령을 실행하여 WebPack을 *릴리스* 모드로 실행합니다.
...
1. `http://localhost:<port_number>`에 대해 브라우저를 엽니다. *wwwroot/index.html* 파일이 제공됩니다. 주소 표시줄에서 URL을 복사합니다.
