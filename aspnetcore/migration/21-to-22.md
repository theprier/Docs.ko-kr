...
앱을 호출 하는 경우 <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> 에 `CreateDefaultBuilder` 메서드의 `Program` 클래스를 호출 `ConfigureKestrel` 대신 Kestrel 서버를 구성 하려면 `UseKestrel` 사용 하 여 충돌을 방지 하기 위해를 [IIS에서 프로세스 호스팅 모델](xref:fundamentals/servers/index#in-process-hosting-model):
...
앱을 호출 하지 않으면 `CreateDefaultBuilder`, 호출 <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **하기 전에** 호출 `ConfigureKestrel`:
...
* 공급자 초기화가 로깅 이동 `Startup.ConfigureServices`합니다.
...
* 필터링을 사용 하 여 변경 된 <xref:Microsoft.Extensions.DependencyInjection.LoggingServiceCollectionExtensions.AddLogging*> 작성기입니다.
...
* 구성 로드를 사용 하 여 변경 된 `AddLogging` 작성기입니다.
