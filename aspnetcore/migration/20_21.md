...
ASP.NET Core 2.1 제공 [ASP.NET Core Id](xref:security/authentication/identity) 로 [Razor 클래스 라이브러리](xref:razor-pages/ui-class)합니다. 생성 된 Identity 코드 2.0 템플릿 변경 내용을 상당한 수행 하지 않은 경우 다음 업그레이드가 고려 하십시오.
